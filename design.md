...
- **画布底栏触发器（既定规格）**：素材节点配置面板底栏的**同类触发器** —— 模型选择、音色、参数摘要 —— 统一为**胶囊圆角**，并共用同一份共享样式与状态契约（`components.css` 的「底栏触发器共享规格」选择器组，五态含 hover / open / active / focus-visible / disabled）；新增同类触发器必须加入该共享组，禁止各自手写外观。已收敛范围为图片 / 视频节点；文本 / 转写（ASR）节点底栏仍是「生成方式分段器 + 模型选择器」并存，尚未纳入该共享组，收敛前本条与下一条红线不在该分支上判违规。
- **红线约束**：严禁在同一工具栏中混用 `9999px`（胶囊圆角）与 `8px`（标准圆角）。胶囊圆角仅限特定的过滤 Chip、状态 Badge 与上一条的底栏触发器。
