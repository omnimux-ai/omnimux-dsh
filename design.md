...
- **画布底栏触发器（既定规格）**：素材节点配置面板底栏的三个触发器 —— 模型选择、音色、参数摘要 —— 统一为**胶囊圆角**，并共用同一份共享样式与状态契约（`components.css` 的「底栏触发器共享规格」选择器组，五态含 hover / open / active / focus-visible / disabled）；新增同类触发器必须加入该共享组，禁止各自手写外观。
