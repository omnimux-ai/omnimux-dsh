...
- **画布底栏触发器（既定规格）**：素材节点配置面板底栏的**同类触发器** —— 模型选择、音色、参数摘要 —— 统一为**胶囊圆角**，并共用同一份共享样式与状态契约（`components.css` 的「底栏触发器共享规格」选择器组，五态含 hover / open / active / focus-visible / disabled）；新增同类触发器必须加入该共享组，禁止各自手写外观。四类模态（图片 / 视频 / 文本 / 转写）已全部收敛：转写节点的模型触发器走 `CustomSelect` 异构结构，其几何与五态由 `.wf-custom-select-trigger.wf-param-bar__select--model` 一并对齐，并由测试锁定不得漂移。
- **画布底栏分段控件（既定规格）**：底栏与浮层内的分段控件（生成方式 / 清晰度 / 有声等）容器与内块统一为**胶囊圆角**，与触发器同一圆角语言；高度契约保持容器 `32px` / 内边距 `2px` / 内块 `26px`；令牌一律带画布 fallback，宿主令牌缺失时不得丢失底色与描边。
- **红线约束**：严禁在同一工具栏中混用 `9999px`（胶囊圆角）与 `8px`（标准圆角）。胶囊圆角仅限特定的过滤 Chip、状态 Badge 与上两条的底栏触发器、分段控件。
...
|---|---|---|---|
| `--dsw-alias-bg-base` / `--dsw-alias-bg-primary` | 页面主背景底色、Stage 根画布 | `#111113` / `#141416` (DSH 原生纯黑) | `#ffffff` |
...
|---|---|---|---|---|
| **画布底色** | 工作台与 Stage 根底色 | DSH 原生曜石纯黑 `#111113` | 纸白 / 极浅暖灰 `#FFFFFF` / `#F8F8F9` | 70% 视网膜纯净底色（绝无偏蓝暗色） |
| **点阵坐标** | 画布 20px 网格微米点 | `rgba(255, 255, 255, 0.07)` 径向消隐 | `rgba(0, 0, 0, 0.05)` 径向消隐 | 弱感知工程参照 |
| **常规卡片** | 节点容器、属性抽屉、提示词面板 | DSH 原生纯黑灰容器 `rgba(255,255,255,0.04)` / `#18181b`，边框 `rgba(255,255,255,0.08)` | 纯白卡片 `#FFFFFF`，边框 `rgba(0,0,0,0.08)` | 保持纯正黑白中性，杜绝偏蓝暗石板色 |
| **视听媒体卡** | **视频播放器、首尾帧预览节点** | 极暗纯黑 `#0d0d0e` / `#111113`，边框 `white/12%` | **依然保持深暗底（#111113）！** | **暗房原则：浅色下视听节点不漂白，杜绝冷蓝底色** |
...
- **例外**：画布底栏的同类触发器（模型选择 / 音色 / 参数摘要）按 §2.2 采用胶囊圆角与共享五态契约（内边距 `0 8px 0 10px`；实现见 `components.css` 的「底栏触发器共享规格」）。
- **Popover 菜单面板**：
