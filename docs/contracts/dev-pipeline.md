...
隔离 worktree 中的相关自动化测试、静态检查与独立评审 → PR required CI / Merge Queue → 合入 `main` → 按变更面物化 Dev、运行与验收。**不存在合并前独立 App/Host 测试环境，不得换名保留。** 自动化测试可使用进程、临时目录和合成 fixture；这些不是独立部署环境，也不证明 Dev 通过。
