...
- 自动化测试使用合成数据；工作树一次性测试进程的配置准备遵循 [plugin-qa](plugin-qa.md#工作树测试配置准备)，不创建常驻预合并环境、不复制共享 profile。显式真实模型测试的 Dev 凭据只读复用必须由当前任务授权覆盖；涉及真实数据的 Dev 验收限定授权范围，不反向写 Prod。密钥不得写入仓库或证据。
