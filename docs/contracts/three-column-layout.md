...
## 九、第 1 轮独立审查退修补充（Issue #2608，全部为新增澄清，不改既有行）

第一轮独立代码审查提出 5 条高、6 条中意见，退修后以下口径成为契约的一部分：

1. **拖拽地板与稳态地板统一（补充 INV-13 / INV-15）**：`CONVERSATION_WIDTH_MIN_PX`（320px）
   已随比例制删除，拖拽分支与稳态分支共用唯一地板 `CONVERSATION_MIN_CHAT_PX`（360px）。
   第 3 节 INV-13 口径澄清段中的「320px 是拖拽分支地板」自本次退修起不再适用；380px 兜底不变。
   若两处地板不一致，拖到 `[320, 360)` 后松手会跳回 360px，与「结算重算对可见宽度是恒等变换」冲突。
2. **迁移前提必须是「用户自己写出的」版式（补充第八节末段）**：外壳 `panels.rightbar` 的写入者
   有两个——用户拖分隔线与**本插件的协调写**（`reconcileRightbarFromRatio` /
   `ensureHealthySplitWidth`）。因此「它是数字」**不构成**迁移前提，判据改为两个用户信号之一：
   ① 本会话观察到外壳分隔线拖拽标记（`data-dragging` / `body[data-dsh-sidebar-dragging]`，只有
   用户手势会写）；② **首次**读到的该字段已是数字，且那一刻本插件尚未写过它。插件每次写该字段
   都必须登记（`notePluginPanelWidthWrite`），否则迁移会把插件自己写出的几何当成用户版式反推
   （自证读数，禁改清单第 6、8 条）。
3. **拖拽期判据与外壳同源（补充 INV-16）**：协调写的「拖拽期绝不写」判据必须包含
   `isShellSplitDragging`（`body` 标记 **或** 任意 `[data-dragging]`，外壳右分隔线拖拽把标记打在
   frame / 手柄上）。只看工作台面板级 `[data-dragging]` 会漏判，从而在指针拖拽进行中写
   `panels.rightbar`、与指针抢同一根轨道。
4. **结算只发生在真实拖拽结束之后（补充第八节状态机 `settling` 行）**：`document` 级 `pointerup`
   会在任意点击时触发，因此结算入口必须同时满足三条：① 本次释放结束了一次被观察到拖拽（拖拽
   观察计数前进）；② 释放当帧外壳已不在拖拽态；③ 处于三栏分栏态（中栏收起 / 右栏收起 / gui
   单栏态一律不结算、不落盘）。缺少任一守卫都会用一个与用户版式无关的比例覆盖用户既有比例。
5. **比例解析唯一真源（补充 INV-17 / INV-15）**：`resolveConversationRatio`（`sidebar-toggle-topbar.js`）
   是存储 → 迁移 → 默认的唯一实现，`workbench/geometry.js` 必须调用它，不得自行
   `readChatRatio() ?? 默认`——否则老用户首开时中栏按迁移值、右栏按默认值，「三栏之和 = 视口」
   当场破裂。解析结果可按存储写入版本号缓存，任何一次真实落盘都使缓存失效。
6. **宽度口径与外壳同源（补充 INV-15 / INV-16）**：参与「视口 − 左栏 − 第三轨」与舞台分母的宽度
   一律取 frame 实测宽（外壳 `computeDesktopColumns` 与把手 `left = viewport − rightbar` 用的就是它），
   测不到才退回窗口内容宽；不得在同一算式里混用 `innerWidth` 与 frame 口径。
7. **缩放档位证据必须可证伪（补充第六节验证矩阵）**：缩放档位夹具的 authored 第三轨不得由期望
   中栏宽推导（否则规则命中与未命中逐轨同值，判定恒真）。除「实测中栏 = 期望」外，还必须以
   变量哨兵证明钉宽规则确实绑在 `--omnimux-conversation-width` 上。
