...
离线测试须覆盖选择顺序、跨项目与重启恢复、偏好读写竞态、目录延迟、零候选、其他节点不变、输入完整性及各提交入口。合入前完成相关静态检查与独立评审，并在自身隔离 worktree 内用真实 ego-browser 或 worktree 隔离 Web QA 运行器完成 UI 验收；没有合入前独立运行环境。Dev 真机验收归人工，不作为 Agent 交付卡点。单测或 HTTP 探活不能代替交互证据。
