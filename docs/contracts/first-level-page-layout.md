...
### 5. 分阶段验证与 Dev 物化
- UI 布局调整先在隔离 worktree 完成相关自动化/静态检查和独立评审，并在自身隔离工作树内完成真实浏览器 Web 验收；没有合入前独立 App/Host 环境。经 required CI/MQ 合入后，收尾必须从已合并 `main` 通过[正式同步入口](ops-entry.md)把本次改动装进 Dev；Dev 真机验收为人工职责，不作为 Agent 交付卡点。
