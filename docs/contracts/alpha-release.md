...
插件阶段及工具归属唯一名单为 [plugin-lifecycle.json](../../plugins/omnimux/src/plugin-lifecycle.json)。Alpha 包括账号、发布、数据分析、任务表单、自动化；项目（工作流／画布）不属于 Alpha，开发版和正式版均保留；未列为 Alpha 的插件不显示此标记。中枢侧栏消费同一名单，在已有入口旁显示 Alpha，说明内测状态、开发优先级及正式版不包含该功能。原有点击、登录、选中和关闭行为保持不变。
