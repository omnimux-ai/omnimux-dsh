...
4. **统一几何与圆角（Geometry Consistency）**：
   ...
   - 严禁在同一工具栏混用 `borderRadius: 999`（胶囊）与 `borderRadius: 8`。画布底栏的**同类触发器**（模型选择 / 音色 / 参数摘要，含转写节点的异构触发器）与**分段控件**（生成方式 / 清晰度 / 有声）统一为胶囊圆角，复用同一份共享样式与状态契约；四类模态（图片 / 视频 / 文本 / 转写）已全部收敛。
...
- [ ] **分阶段验证**：隔离 worktree 完成相关自动化/静态检查与独立评审，并在自身隔离工作树内用 ego-browser 或 worktree 隔离 Web QA 运行器（动态端口、自清理，保留截图/结构化报告）完成真实浏览器 Web 验收。插件改动合入后按[开发环境合同](dev-pipeline.md)由收尾自动装进 Dev；Dev 45120 真机验收归人工，不作为 Agent 交付卡点。仅平台/壳层行为追加 Electron 证据；不将未合并源码送入 Dev/Prod，不默认重启或写生产。
