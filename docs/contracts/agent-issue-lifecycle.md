...
- 每个任务只执行适用的 DoD。纯文档、流程或脚本无需 App 物化或浏览器验收；合并前不建独立运行环境，未触及 UI 的逻辑变更不伪造浏览器证据。Agent 侧验收在自身隔离 worktree 内完成真实浏览器 Web 验证；Dev 真机验收为人工职责，不作为 Agent 交付卡点。
...
| 合并后交付 | merge commit；隔离 worktree Web 验证证据与截图保留；Dev 物化按需（保留给人工查看或人工要求时） | 隔离 worktree Web 验证证据留存，或明确无需 App 物化，再完成安全清理；Dev 真机验收归人工，不阻塞、不等待、不声称取得 |
...
| Host / 插件运行行为 | 合并前相关自动化测试、静态检查与独立评审；隔离 worktree 内的真实运行验证；Dev 物化按需（保留给人工查看或人工要求时），Dev 真机验收归人工 |
| Client / Stage / 侧栏 | 合并前相关自动化/静态检查与独立评审；隔离 worktree 真实浏览器 Web 验证（ego-browser 或 worktree 隔离 Web QA 运行器）+ 截图/结构化报告；Dev 真机验收归人工，不作为 Agent 卡点 |
