...
- 实施、修复指令授权范围内的本地调查、隔离实施、验证和修复；远端 Issue/分支/PR 写入、合并、Dev 物化/重启和生产发布须有覆盖相应动作的明确授权。同一指令可以明确授权多个动作；已有授权跨轮持续有效，不要求特定口令。测试通过证明质量，不补足缺失权限。
- 仅有本地授权时，完成 diff、适用检查及 Issue/PR 文案，再询问剩余远端动作。明确要求创建 PR 包含必要的任务分支 push；明确要求合并包含必要的分支/PR 准备。合并不自动授权共享 App 重启或生产发布。用户明确要求“仅提方案/仅开 PR 暂不合并”时，严格停在对应范围。
- 测试、L2 或浏览器验收失败阻止合并和交付声明；Agent 在既有范围和风险内继续诊断、修复、复验。只有缺少必要授权、输入或不存在合规可行路径时，暂停受影响动作并报告；继续独立授权工作。适用质量门禁与授权均满足后，主动加入 Merge Queue 并核实结果，不重复索要同一批准。
- 合入授权不等于生产发布授权。`~/.omnimux`、`--prod`、`--all`、正式包发布与回滚必须获得单独、明确的发布授权；开发交付的物化目标仅为 `~/.omnimux-dev`，执行前核对该动作已有授权。
- 公共 App 重启授权须覆盖具体 App（Dev 或 Prod）及协调窗口；已有有效授权时不重复询问。确认后由 Agent 完成非付款步骤，不得要求用户代点或代跑命令；不得默认强杀不明进程。
...
交互式通道按上方「授权边界」核对用户已批准的动作与适用证据；同一任务的有效授权无需重复确认。实施指令或测试通过都不自动授权远端写入、合并或物化。可恢复的验收失败继续在授权范围内修复，仅阻止相关合入或交付声明。不得把用户授权伪装成 `/auto-approve` 评论，也不得调用无人值守通道绕过 R0/R1 限制。
...
合并确认后，Agent 在主检出干净且适合快进时同步本地；已有用户改动须保留。完成已授权的 Dev 物化及适用的 45120 验收后，再清理任务所属 L2 与 worktree。缺少物化/重启授权时只询问该动作；生产发布需单独授权。物化失败或必要证据不完整时保留现场并继续可行修复，不得把“已合并”写成“已交付”。
