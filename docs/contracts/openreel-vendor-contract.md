...
在常规 `plugin-qa` 之上，`omnimux-clip` 专项：源码/自动化可验证项合入前检查，真实 GUI、播放、导出与生命周期证据在 Agent 自身隔离 worktree 内用 ego-browser 或 worktree 隔离 Web QA 运行器验收（动态端口、自清理，保留截图/结构化报告）；Dev 真机验收归人工，不作为 Agent 交付卡点。以下 FAIL 阻止对应阶段通过，不要求合入前独立运行环境。
...
4. **复检**：合入前在隔离 worktree 完成相关自动化/静态检查与独立评审；Agent 侧验收按 §5 在自身隔离 worktree 内用 ego-browser 或 worktree 隔离 Web QA 运行器逐项完成（动态端口、自清理，保留截图/结构化报告），Dev 真机验收归人工，不作为 Agent 交付卡点。无合入前独立运行环境；拿不到**隔离 worktree Web 验证证据**不得声明交付完成。
