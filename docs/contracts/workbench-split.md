...
`open()` sequence: capture the current session/provider/layout/stage → require a current session (else `false` + toast) → wait for registration and a sidebar snapshot with the same `sessionId` → capture existing empty Files seed IDs → await `openTab({ type, id, path: sentinel })` → verify success and unchanged context → close only captured, still-empty Files seed tabs → `closeDetails` → **release the captured current `data-dsh-product-stage`**. A briefly lagging sidebar snapshot may catch up within the wait; timeout or context drift returns `false` without plugin-owned cleanup or focus changes. Missing `sessionId` is not readiness. Provider-owned partial side effects are not transactionally rolled back. If `preserveLayout` (Agent): stop; do not `setFocus`. Otherwise apply a contract-authorized explicit `focus` when provided; else apply Default Focus Rule or restore `(sessionId, tabId)` memory, writing geometry with `persistUserIntent: false`, and if the target is `gui` also enter host right-panel fullscreen. NEVER `claim` a stage. NEVER `sessions.create({})`. NEVER persist `explicit: true` on this default path.
