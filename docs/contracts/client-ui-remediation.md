...
| P0.2 | 工程师 | 1d | **Products** 拆 L2 Action Row、Refresh→IconButton、补 sort/view 或显式「无类型则 chips=全部」 | 合入前相关自动化/静态与独立评审；隔离 worktree Web 验证 + 四层截图（Dev 真机验收归人工） |
...
每 PR 合入前回归：相关插件测试、静态检查与独立评审；合入后在隔离 worktree 内用 ego-browser 或 worktree 隔离 Web QA 运行器检查打开/关闭/空态/主 CTA（Dev 物化按需、仅供人工查看）。**不得**顺手改 Host 路由 / 工具 schema。
