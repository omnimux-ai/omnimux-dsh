...
- CLI 返回 pending/request path 只表示请求已准备，不是通过。当前 IAB Tab 必须实际执行 `scripts/codex-browser-qa.mjs` 的 `runPreparedQa(requestPath, { tab })` 并产出最终报告；它不会自行取得 L2 登录入口或执行 token 登录导航。
...
| 任一页面状态下同源请求返回 401/403 | 返回需要认证；L2 仅可由显式 `openL2IabPage` 读取当前 Host 的正式链接并完成一次登录导航，再重新准备页面 |
