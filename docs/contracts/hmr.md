...
合并前在隔离 worktree 运行相关自动化/静态检查与独立评审，并在自身隔离 worktree 内完成真实浏览器 Web 验收（ego-browser 或 worktree 隔离 Web QA 运行器，动态端口、自清理），不修改官方源码或生产 profile。Dev 真机验收归人工，不作为 Agent 交付卡点；`pnpm verify:live <stage>`（Dev 45120）属人工侧程序，但依据 [插件 QA](plugin-qa.md) 的核对待办如下：
