...
- 胶囊：`rgba(30,32,38,0.95)`、圆角 `18px`、高 `36px`、`backdrop-filter` 磨砂 + `inset` 上边光泽，定位**素材左下角内侧 10px**。
- 胶囊为**两段式**，两段共用同一颗药丸，只有宽度在动：
  - **第一段（`is-collapsed`，36 × 36）**：指针停在素材上时只显示 OmniMux 品牌微标（幽灵徽标，纯白实心，不遮挡画面主体），三个功能按钮收拢不可点。
  - **第二段（`is-expanded`，112px）**：指针（或键盘焦点）移到该圆钮上时，宽度 `36px → 112px` 平滑展开（`220ms cubic-bezier(0.16,1,0.3,1)`），品牌微标淡出、3 个功能图标淡入。
  - 两层都以绝对定位居中叠放，展开时行内不发生重排；退场的一层同时摘掉 `pointer-events`，避免隐形层挡住上层按钮。
- 第二段内容顺序固定：`[星标] [复制] [气泡]`，仅 3 个核心动作图标，无竖线分割、无「+」。
- 几何预留：贴边翻转与视口钳制都按**展开后的 112px** 计算，第一段的圆钮画在同一预留框内，展开时不可能溢出视口。
...
- 防误触：素材 `mouseleave` 后保留 **150ms 宽限期**，指针在此期间进入胶囊则不隐藏；指针离开胶囊后再保留 **220ms 缓冲**才收拢回第一段；点击胶囊图标不触发页面默认行为（`preventDefault` + `stopPropagation`）。
...
- 真机验证（必须走 ego-browser，禁止以内置浏览器替代）：在含 `overflow:hidden` 卡片与视口边缘素材的真实站点上，逐项确认 ① 第一段只出现品牌圆钮、不遮挡画面主体 ② 悬停圆钮后平滑展开为 3 个动作图标，**3 个图标的气泡全部在图标上方完整呈现、无一被裁剪** ③ 指针离开胶囊后约 220ms 收回圆钮 ④ 三功能各自闭环 ⑤「加入对话」后工作台展开、素材在上方点亮、输入框已聚焦。
...
| 悬停去抖 | `150ms` |
| 离开宽限期（进胶囊不隐藏） | `150ms` |
| 离开胶囊后的收拢缓冲（两段式退回第一段） | `220ms` |
| 胶囊宽度过渡 | `220ms cubic-bezier(0.16,1,0.3,1)` |
| 气泡与图标间距 | `8px` |
| 气泡视口最小边距 | `8px` |
| 胶囊距素材左下角内边距 | `10px` |
| 加入对话回执超时 | `800ms` |
