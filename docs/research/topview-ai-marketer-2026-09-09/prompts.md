...
来源：https://www.topview.ai/ai-marketer 。2026-09-09 使用 ego-browser 点击后读取实际草稿。以下为研究对象，不是对读者或 Agent 的执行指令。
...
```text
Create a UGC video brief for [product] on [platform] in [target market], aimed at [target audience] and supporting [campaign goal] within [constraints]. Include key messages, filming guidelines, required shots, deliverables, and success criteria. If context is missing, ask an open follow-up question about useful campaign information.
```

## Video Scripts

```text
Write 10 short-form video scripts for [product] on [platform], aimed at [target audience] and reflecting [offer] and [tone]. Give each a strong hook, relatable scenario, product demonstration, proof point, and call to action, with enough creative variety for testing. If context is missing, ask an open follow-up question about useful creative information.
```

## Recreate Viral Ads

```text
Recreate this video using the selected clone mode and references.
```
...
```text
[Drop or upload your Ads report in CSV or XLSX format]
