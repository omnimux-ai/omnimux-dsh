...
export function validateLiveQaReport(report, request, expected = {}) {
  ...
  assert.equal(report.tool, 'ego-browser', 'Wrong browser evidence tool')
  ...
  assert.ok(typeof report.tabId === 'string' && report.tabId.length > 0, 'Missing real ego tab id')
  assert.ok(Number.isSafeInteger(report.taskSpaceId) && report.taskSpaceId > 0, 'Missing isolated ego task space id')
  ...
  assert.ok(!actual.username && !actual.password && !actual.search && !actual.hash, 'Reported ego URL contains credentials or query data')
