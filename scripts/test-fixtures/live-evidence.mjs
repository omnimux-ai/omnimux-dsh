...
export function liveEvidence(root, evidenceDir) {
  ...
  const request = {
    ...
    target: 'dev', profile: 'omnimux-dev', url: 'http://127.0.0.1:45120/', stage: 'assets', targets: [target],
    evidenceDir, createdAt: new Date(Date.now() - 2_000).toISOString(), consumedAt: now,
  ...
  const bundle = plugin => ({
    ...
    loadedScriptUrl: `http://127.0.0.1:45120/plugins/${plugin}/client.js`, loadedScriptSha256: 'e'.repeat(64),
  ...
  const proof = { requestedOrigin: 'http://127.0.0.1:45120', target: 'dev', bundles: [bundle('omnimux'), bundle('omnimux-assets')] }
  const report = {
