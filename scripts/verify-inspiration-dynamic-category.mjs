 ...
 *      不再是写死的 9 个中文电商类目；
 *   3. 聚合端点失败时下拉降级为仅「全部」，页面无报错；
 *   4. 截图证据落盘 docs/evidence/inspiration-dynamic-category-filter-verified.png。
 */
 ...
 * @param {{ failCategories?: boolean }} options
 */
  ...
  const server = http.createServer((req, res) => {
    ...
    if (url.pathname === '/omnimux/inspiration/categories') {
      if (options.failCategories) {
        json(500, { error: 'upstream walk failed' });
        return;
      }
      json(200, { data: CATEGORY_ROWS });
      return;
    }
...
async function scenarioDegradeOnFailure(bundledJs, report) {
  const server = await startServer(harnessHtml(bundledJs), { failCategories: true });
  const { proc, cdpPort } = await launchChrome();
  let client = null;
  try {
    client = await connectCdp(cdpPort);
