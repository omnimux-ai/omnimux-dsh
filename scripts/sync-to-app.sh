#!/bin/bash
# sync-to-app.sh — 合并后同步：先构建产物，再物化进目标 profile。
#
...
DEFAULT_PLUGINS=(omnimux omnimux-forms omnimux-accounts omnimux-assets omnimux-products omnimux-workflow omnimux-market omnimux-inspiration omnimux-clip omnimux-video omnimux-video-preview omnimux-analytics omnimux-publish omnimux-automation omnimux-browser omnimux-viewer omnimux-apps omnimux-social-harvest)
...
✓ 已完成物化到目标 Profile（零副作用，未重启任何进程）。
  ...
  - 【安全红线】不得强杀不明进程；App 重启须先核实目标、授权和使用冲突。
EOF
