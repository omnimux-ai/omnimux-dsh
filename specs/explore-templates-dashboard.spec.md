...
- **悬停状态 (Hover)**：
  - 标题向上平滑位移 50px；
  - 底部升起深灰半透明毛玻璃胶囊按键（直接复用 `.omnimux-trending-recreate-btn` 样式，绝无蓝色）：
    - 模板与 TikTok 热门卡片文案为「↺ 复刻」；
    - Skills 技能库卡片文案为「↺ 使用」（对齐用户指示）。
...
### 2.5 查看全部全量数据展示、首屏分批与多级缓存机制
1. **全量数据覆盖**：
   - 点击任一分类胶囊或货架行「查看全部」时，展开展示该分类全部数据（覆盖模板 395 套、Skills 70 套、TikTok 热门全量视频）；
2. **首次加载分批 (Initial Batching)**：
   - 进入全量大网格时，首屏先精准渲染前 16 项（4 列 × 4 行），避免大量节点并发挂载造成卡顿；
   - 触底时（通过轻量 Sentinel 观察器或加载触发），平滑无感追加下 16 项，直到全部加载完毕；
   - 底部提供「已呈现该分类全部 N 项数据」状态指示；
3. **内存级缓存 (Memory Cache) 与请求节流**：
   - 前端维护 `GRID_DATA_CACHE = new Map()`，以 `categorySlug` 缓存已解析数据与分页游标；
   - 用户在「全部货架」与「各分类查看全部」之间来回切换时直接命中内存缓存，**0ms 秒开**；
   - 远程接口采用短期 TTL（5 分钟）与条件校验，同一周期内不重复发起网络请求，彻底节省服务器带宽与 API 配额消耗；
4. **媒体懒加载**：
   - 全量卡片图片严格配置 `loading="lazy"` 与 `decoding="async"`，视口外卡片不提前发起网络图片请求。
