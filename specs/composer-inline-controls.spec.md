...
## 首轮退修验收补充
- 在真实 700×850 视口、318px 输入框内打开参数菜单：内容由面板自身纵向滚动承载，无水平溢出或越界覆盖链接输入区；底部时长可滚动到达。
- 比例与模型版本使用原生键盘可达控件，Tab/Enter/Space 可选择；品牌/型号聚焦更新级联，Escape 返回触发器。默认媒体面板保留原尺寸和参数状态，仅补齐同一共享控件的键盘语义。
- 测量限于相关输入区变动；恢复、正常/短文字测量和最终几何写入按批执行。停靠激活时尊重既有停靠几何所有权，不用旧 seat 覆盖 dock。
- 样式的幂等存在检查与组件持有生命周期分离；单测座位/会话列分别模拟，每例清理临时覆盖。
- 私有验证脚本对依赖/产物/可执行文件缺失给出明确错误，保留失败累积、完整断言集合、实际加载身份及失败清理证据。不改共享测试启动合同或官方宿主。
- 浏览器预验通过后再固化本轮E2E；历史正文和同页会话仅使用冷启动前私有合法记录。无法证明的缩放等覆盖明确保留缺口。
