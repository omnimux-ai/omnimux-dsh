...
- **AC-4**：数据分析看板中，仅保留标签栏下方的一条分割线，筛选栏下方无任何底边框或分割线（`border-bottom` 为 0 或 none）。
  - **本条「标签栏下方」的位置描述已被 #1869 按用户明确要求覆盖**：数据分析看板的分割线改到标签栏**上方**（`PageHeader` 与 `ActionNavRow` 之间），标签栏与筛选栏之间不再有分割线。本条的实质要求（筛选栏下方无任何底边框/分割线、全页仅此一条分割线）仍然成立并未被放宽；仅数据分析页的分割线垂直位置以 #1869 为准。产品库、资产库、技能市场不受影响。
