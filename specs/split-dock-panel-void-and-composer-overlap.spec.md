...
> **适用范围修订（2026-09-15，Issue #1821 / PR #1825）**：本文件 AC-2 与 AC-8 描述的
> 「收起态下输入框悬浮于画布底部并可用」现仅适用于**图像画布**舞台；投影条件已由
> `specs/canvas-composer-display-scope.spec.md` 收敛为「图像画布身份 + 右侧栏全屏」。
> 其余插件页（项目、技能/专家、产品库…）收起态**不再投射**输入框。面板几何条款
> （AC-1 / AC-4 / AC-5 等）不受影响，继续有效。
