...
5. Sync the primary checkout with `git pull --ff-only origin main`. With uncommitted changes, first prove incoming paths do not overlap and record their content/modes; verify they are unchanged afterwards. If safety cannot be established, keep the primary intact and deliver from the verified task worktree. Never stash/reset another task to make synchronization possible. Pure instruction/docs changes need no App materialization. For runtime changes, complete Dev materialization and task-specific acceptance under the task authorization before cleanup; independently verify restart target and usage conflicts as required by the Git/PR policy.
