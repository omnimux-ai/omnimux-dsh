...
3. Follow the Git/PR policy for remote writes and merge. A request to merge includes normal branch/PR preparation; missing paperwork is not a reason to hand the workflow back. With local-only authorization, finish the diff, checks, and Issue/PR text before asking for remote writes. With PR authorization but no merge authorization, finish the reviewable PR and ask only for merge. Passing checks does not create authorization.
...
5. Sync a clean primary checkout with `git pull --ff-only origin main`; preserve existing user changes if it is dirty. Pure instruction/docs changes need no App materialization. For runtime changes, verify authorization covers the Dev target and any shared-App restart, then complete materialization and task-specific acceptance before cleanup.
