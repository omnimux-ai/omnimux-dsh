...
3. Fetch the named base (normally `origin/main`). Verify and reuse the current task's isolated worktree. Only create one if none exists or its isolation is unsuitable, using `bash scripts/worktree.sh new <plugin-or-common>-<topic> origin/main --type agent --issue <issue-id>` from the product root. Keep the worktree inside `<repo>/.worktrees/`; do not use the legacy sibling-directory `git-wt.sh` entry. Do not switch the primary checkout for review. The alias `pnpm wt:start <task> [base] --type agent --issue <issue-id>` maps to the same `worktree.sh new` entry; use an ASCII kebab-case task such as `<plugin-or-cross>-<topic>-issue-<id>`.
...
- Before merge, run relevant automated tests/static checks and independent review in the isolated worktree, then satisfy required CI and Merge Queue. Do not create a separate pre-merge App/Host environment. For UI/Stage changes, follow [plugin QA](../../../docs/contracts/plugin-qa.md) after merge and Dev materialization: load ego-browser, run the shared `verify:live` probe on Dev 45120 in the current isolated ego task/Tab, and preserve same-run identity, runtime proof, and real PNG evidence. Missing ego capabilities are BLOCKED; do not fall back to IAB. Dev/Prod must not link or receive unmerged worktrees; CI `qa:pass` does not prove Dev acceptance.
- When the pipeline returns `ready-for-agent` (or legacy `ready-for-boss`), retain ownership: read its phase, PR, revision, risk and reports; recheck live authorization/revocation, required checks and independent review. For applicable Dev delivery, confirm the merge first, then materialize and verify the merged version before cleanup or declaring success. Do not rerun implementation or treat the handoff as human approval. `--manual` does not grant permission and `--no-merge` / `--no-materialize` must remain respected.
...
5. Sync the primary checkout with `git pull --ff-only origin main`. With uncommitted changes, first prove incoming paths do not overlap and record their content/modes; verify they are unchanged afterwards. If safety cannot be established, keep the primary intact and continue non-materialization work in the task worktree. Sync still requires the clean `main`, exact `origin/main`, and source identity in [dev pipeline](../../../docs/contracts/dev-pipeline.md). Never stash/reset another task to make synchronization possible. Pure instruction/docs/process/script changes need no App materialization. A merged plugin change must be installed into Dev by the closeout command below; do not pull and stop. Independently verify restart target and usage conflicts as required by the Git/PR policy.
6. Preserve evidence, enumerate the exact task-owned worktree/branch/files, and verify they contain no unrelated work. Then run `bash scripts/worktree.sh remove <task> --pr <pr-number>` from the primary checkout; do not use a force flag or broad cleanup. That remove installs the merged plugin change into Dev even when main was already current, and skips only when Dev already matches.
