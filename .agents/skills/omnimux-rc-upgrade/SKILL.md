...
5. Follow [plugin QA](../../../docs/contracts/plugin-qa.md) for relevant automated tests/static checks and independent review in the isolated worktree before required CI/Merge Queue. There is no separate pre-merge App/Host environment. After authorized merge/materialization, verify the exact delivered Dev App (45120) with ego-browser and the shared probe. Load ego-browser first; missing capabilities are BLOCKED, never an IAB fallback. Test shell-specific behavior in Electron, including Host launch with `--no-open`.
