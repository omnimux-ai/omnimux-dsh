...
describe('picker-model generalized max', () => {
  ...
  it('AssetPicker uses the kit lg tier + shared dialog contract instead of a dead width prop', async () => {
    ...
    assert.ok(source.includes('ASSET_CARD_CSS'), '卡片样式来自 AssetPickerCard，弹窗只组合引用')
    const cardCss = readFileSync(new URL('./AssetPickerCard.jsx', import.meta.url), 'utf8')
    assert.ok(cardCss.includes('aspect-ratio: 1 / 1'), '卡片缩略图为 1:1')
