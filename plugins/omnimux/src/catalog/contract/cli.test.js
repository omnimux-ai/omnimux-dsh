...
test('runVerify audit ok; strict ok with 79 dispositions resolved', async () => {
  const silent = () => true;
  ...
  try {
    ...
    assert.equal(strict.dispositions.total, 79);
...
test('runVerify JSON report exposes schemaVersion only (no root version)', async () => {
  ...
  try {
    ...
    assert.equal(parsed.dispositions.total, 79);
  } finally {
    process.stdout.write = origWrite;
