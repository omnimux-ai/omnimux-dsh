/**
 * 工作台词条。原生任务树、更新提示与设置页相关词条已随架构退役，不在本文件出现。
 */

export const NS = "omnimux-automation"

export const en = {
  "workbench.offline": "Host is unreachable. Retry when the connection recovers.",
  "overview.hint": "Toggle on the right to pause or resume a task.",
  "runs.unread": "New result",
  "runs.hint": "Each row opens the session that ran it.",
  "nav": "Automation",
  "tab": "Scheduled tasks",
  "sidebar.views": "Scheduled views",
  "sidebar.viewOverview": "Task overview",
  "sidebar.empty": "No scheduled runs yet.",
  "menu.run": "Run task",
  "menu.edit": "Edit task",
  "menu.delete": "Delete task",
  "time.withinMinute": "about {count} minutes",
  "time.withinHour": "about {count} hours",
  "time.withinDay": "about {count} days",
  "action.create": "Create",
  "action.createMenu": "More ways to create",
  "action.chatCreate": "Create in chat",
  "action.manualCreate": "Manual setup",
  "chat.prompt": "I want to create a scheduled task that runs every [interval] and does [the actual task]",
  "tabs.mine": "My tasks",
  "tabs.runs": "Run history",
  "sort.by": "Sort",
  "sort.created.asc": "Created time ascending",
  "sort.created.desc": "Created time descending",
  "sort.planned.asc": "Planned time ascending",
  "sort.planned.desc": "Planned time descending",
  "sort.default.saved": "Default",
  "form.workspace": "Workspace",
  "form.workspacePath": "Folder path",
  "form.workspacePathPlaceholder": "D:\\work\\project",
  "form.model": "Model",
  "form.modelSelect": "Select model",
  "form.effort": "Reasoning",
  "form.effort.none": "Default",
  "form.effort.low": "Low",
  "form.effort.medium": "Medium",
  "form.effort.high": "High",
  "form.error.workspace": "Select a workspace.",
  "modal.title": "New scheduled task",
  "modal.edit": "Edit scheduled task",
  "modal.save": "Save",
  "stats.total": "All",
  "stats.active": "Active",
  "stats.next": "Next run",
  "stats.attention": "Needs attention",
  "stats.noneScheduled": "None scheduled",
  "stats.noAttention": "All clear",
  "section.refresh": "Refresh",
  "runs.empty": "No runs yet. Run one now, or wait for the next scheduled occurrence.",
  "overview.empty": "No scheduled tasks yet. Click New scheduled task above to create one.",
  "form.title": "Create automation",
  "form.subtitle": "Write a complete, standalone task. Scheduled runs do not inherit this conversation.",
  "form.name": "Name",
  "form.namePlaceholder": "Daily regression triage",
  "form.prompt": "Task prompt",
  "form.promptPlaceholder": "Inspect new test failures, locate the regression, and propose a verified minimal fix…",
  "form.schedule": "Schedule",
  "form.planTime": "Schedule",
  "form.hourly": "Hourly",
  "form.monthly": "Monthly",
  "form.custom": "Custom",
  "form.minutesShort": "min",
  "form.daysShort": "days",
  "form.monthDay": "Day {day} of month",
  "form.skills": "Skills",
  "form.skillsEmpty": "No skills available",
  "schedule.hourlyAt": "Every hour at :{minute}",
  "schedule.monthlyAt": "Monthly on day {day} · {time}",
  "schedule.customAt": "Every {count} days · {time}",
  "form.once": "Does not repeat",
  "form.interval": "Interval",
  "form.daily": "Daily",
  "form.weekly": "Weekly",
  "form.runAt": "Run at",
  "form.every": "Every",
  "form.minutes": "minutes",
  "form.time": "Time",
  "form.days": "Weekdays",
  "form.timeZone": "Time zone",
  "form.maxConcurrentRuns": "Concurrency",
  "form.maxConcurrentRunsHint": "Queued and running executions share this limit. Defaults to 1; when full, scheduled triggers are skipped and manual runs are rejected.",
  "form.error.maxConcurrentRuns": "Enter a positive whole number for concurrent runs.",
  "form.permission": "Permission boundary",
  "permission.readOnly": "Read Only",
  "permission.workspaceWrite": "Workspace Write",
  "permission.fullAccess": "Full access",
  "form.cancel": "Cancel",
  "form.submit": "Create automation",
  "form.submitting": "Creating…",
  "form.error.name": "Enter a name.",
  "form.error.prompt": "Enter a complete, standalone task prompt.",
  "form.error.once": "Choose a valid future date and time.",
  "form.error.interval": "The interval must be between 1 and 43,200 minutes.",
  "form.error.weekdays": "Select at least one weekday.",
  "day.1": "Mon",
  "day.2": "Tue",
  "day.3": "Wed",
  "day.4": "Thu",
  "day.5": "Fri",
  "day.6": "Sat",
  "day.7": "Sun",
  "status.active": "Active",
  "status.paused": "Paused",
  "status.queued": "Queued",
  "status.running": "Running",
  "status.succeeded": "Succeeded",
  "status.failed": "Failed",
  "status.skipped": "Skipped",
  "status.cancelled": "Cancelled",
  "status.interrupted": "Interrupted",
  "card.nextRun": "Next",
  "card.lastRun": "Last",
  "card.never": "Not yet run",
  "schedule.onceAt": "Once · {time}",
  "schedule.everyMinutes": "Every {count} minutes",
  "schedule.dailyAt": "Daily · {time}",
  "schedule.weeklyAt": "{days} · {time}",
  "card.pause": "Pause",
  "card.resume": "Resume",
  "card.runNow": "Run now",
  "card.more": "More actions",
  "card.delete": "Delete",
  "card.confirmDelete": "Delete this automation?",
  "card.confirmDeleteHint": "Run history is kept for audit.",
  "card.confirm": "Delete",
  "card.cancel": "Cancel",
  "run.trigger.schedule": "Scheduled",
  "run.trigger.manual": "Manual",
  "run.openSession": "Session {id}",
  "run.markRead": "Mark reviewed",
  "loading": "Loading automations…",
  "error.title": "Automations could not be loaded",
  "error.retry": "Try again",
  "error.action": "The action failed. Please try again.",
  "error.offline": "Could not reach the local automation service. Check that DSH is still running and try again.",
  "time.now": "now",
  "time.minuteAgo": "{count}m ago",
  "time.hourAgo": "{count}h ago",
  "time.dayAgo": "{count}d ago",
  "time.inMinute": "in {count}m",
  "time.inHour": "in {count}h",
  "time.inDay": "in {count}d",
  "list.title": "Scheduled tasks",
  "list.subtitle": "Let an Agent schedule a task, set a reminder, or watch for updates.",
  "list.searchPlaceholder": "Search scheduled tasks",
  "list.clearSearch": "Clear search",
  "list.noMatch": "No tasks match the current filters.",
  "list.clearFilter": "Clear filters",
  "filter.label": "Filter by state",
  "filter.all": "All",
  "filter.active": "Active",
  "filter.paused": "Paused",
  "filter.finished": "Finished",
  "detail.title": "Task detail",
  "detail.close": "Close detail",
  "detail.detailGroup": "Details",
  "detail.scheduleGroup": "Frequency",
  "detail.runtime": "Runtime",
  "detail.runtimeValue": "Each run starts a new chat",
  "detail.runtimeHint": "Scheduled runs do not inherit the current conversation.",
  "detail.defaultModel": "Default model",
  "detail.modelSearch": "Search models",
  "detail.advanced": "Advanced",
  "detail.save": "Save",
  "detail.saving": "Saving…",
  "detail.unsaved": "Unsaved changes",
  "detail.promptExpand": "Show all",
  "detail.promptCollapse": "Show less",
  "detail.discard": "Discard unsaved changes?",
  "detail.discardHint": "Closing or switching tasks drops the edits you have not saved.",
  "detail.discardConfirm": "Discard",
  "detail.keepEditing": "Keep editing",
  "history.title": "Run history",
  "history.expand": "Show all {count}",
  "history.collapse": "Show less",
  "history.empty": "No runs yet.",
}

export const zh = {
  "workbench.offline": "暂时连不上主程序，网络恢复后会自动重试。",
  "overview.hint": "右上角开关可直接暂停或恢复任务。",
  "runs.unread": "有新结果",
  "runs.hint": "点击任意一条可打开对应的执行会话。",
  "nav": "自动化",
  "tab": "定时任务",
  "sidebar.views": "定时页视图",
  "sidebar.viewOverview": "任务总览",
  "sidebar.empty": "还没有定时任务执行记录。",
  "menu.run": "执行任务",
  "menu.edit": "编辑任务",
  "menu.delete": "删除任务",
  "time.withinMinute": "{count} 分钟内",
  "time.withinHour": "{count} 小时内",
  "time.withinDay": "{count} 天内",
  "action.create": "创建",
  "action.createMenu": "更多创建方式",
  "action.chatCreate": "使用对话创建",
  "action.manualCreate": "手动设置",
  "chat.prompt": "我要创建一个定时任务，每【时间间隔】执行【具体任务】",
  "tabs.mine": "我的定时任务",
  "tabs.runs": "执行记录",
  "sort.by": "排序",
  "sort.created.asc": "按创建时间升序",
  "sort.created.desc": "按创建时间倒序",
  "sort.planned.asc": "按计划时间升序",
  "sort.planned.desc": "按计划时间倒序",
  "sort.default.saved": "默认",
  "form.workspace": "工作目录",
  "form.workspacePath": "目录路径",
  "form.workspacePathPlaceholder": "D:\\work\\project",
  "form.model": "模型",
  "form.modelSelect": "选择模型",
  "form.effort": "推理等级",
  "form.effort.none": "默认",
  "form.effort.low": "Low",
  "form.effort.medium": "Medium",
  "form.effort.high": "High",
  "form.error.workspace": "请选择工作区。",
  "modal.title": "新建定时任务",
  "modal.edit": "编辑定时任务",
  "modal.save": "保存",
  "stats.total": "全部",
  "stats.active": "已启用",
  "stats.next": "下次运行",
  "stats.attention": "需要关注",
  "stats.noneScheduled": "暂无计划",
  "stats.noAttention": "一切正常",
  "section.refresh": "刷新",
  "runs.empty": "还没有运行记录。你可以立即运行一次，或等待计划触发。",
  "overview.empty": "暂无定时任务，可点击上方【新建定时任务】开始创建。",
  "form.title": "创建自动化",
  "form.subtitle": "请写完整、独立的任务说明：定时运行不会继承当前对话。",
  "form.name": "名称",
  "form.namePlaceholder": "每日回归测试分诊",
  "form.prompt": "任务指令",
  "form.promptPlaceholder": "检查新增测试失败，定位回归原因，并给出经过验证的最小修复方案……",
  "form.schedule": "运行计划",
  "form.planTime": "计划时间",
  "form.hourly": "每小时",
  "form.monthly": "每月",
  "form.custom": "自定义",
  "form.minutesShort": "分",
  "form.daysShort": "天",
  "form.monthDay": "每月第 {day} 天",
  "form.skills": "技能",
  "form.skillsEmpty": "暂无可用技能",
  "schedule.hourlyAt": "每小时 :{minute}",
  "schedule.monthlyAt": "每月 {day} 日 · {time}",
  "schedule.customAt": "每 {count} 天 · {time}",
  "form.once": "不重复",
  "form.interval": "间隔",
  "form.daily": "每天",
  "form.weekly": "每周",
  "form.runAt": "运行时间",
  "form.every": "每隔",
  "form.minutes": "分钟",
  "form.time": "时间",
  "form.days": "星期",
  "form.timeZone": "时区",
  "form.maxConcurrentRuns": "并发数量",
  "form.maxConcurrentRunsHint": "同一任务的排队及运行次数共用此上限，默认 1；满额时跳过当次定时触发，手动运行将被拒绝。",
  "form.error.maxConcurrentRuns": "并发数量必须为大于等于 1 的整数。",
  "form.permission": "权限边界",
  "permission.readOnly": "只读",
  "permission.workspaceWrite": "工作区写入",
  "permission.fullAccess": "完全访问",
  "form.cancel": "取消",
  "form.submit": "创建自动化",
  "form.submitting": "创建中…",
  "form.error.name": "请输入名称。",
  "form.error.prompt": "请输入完整、独立的任务指令。",
  "form.error.once": "请选择有效的未来日期和时间。",
  "form.error.interval": "运行间隔必须在 1 到 43,200 分钟之间。",
  "form.error.weekdays": "请至少选择一天。",
  "day.1": "周一",
  "day.2": "周二",
  "day.3": "周三",
  "day.4": "周四",
  "day.5": "周五",
  "day.6": "周六",
  "day.7": "周日",
  "status.active": "已启用",
  "status.paused": "已暂停",
  "status.queued": "排队中",
  "status.running": "运行中",
  "status.succeeded": "已完成",
  "status.failed": "失败",
  "status.skipped": "已跳过",
  "status.cancelled": "已取消",
  "status.interrupted": "已中断",
  "card.nextRun": "下次",
  "card.lastRun": "最近",
  "card.never": "尚未运行",
  "schedule.onceAt": "单次 · {time}",
  "schedule.everyMinutes": "每 {count} 分钟",
  "schedule.dailyAt": "每天 · {time}",
  "schedule.weeklyAt": "{days} · {time}",
  "card.pause": "暂停",
  "card.resume": "恢复",
  "card.runNow": "立即运行",
  "card.more": "更多操作",
  "card.delete": "删除",
  "card.confirmDelete": "确认删除这个自动化？",
  "card.confirmDeleteHint": "运行历史会保留用于审计。",
  "card.confirm": "确认删除",
  "card.cancel": "取消",
  "run.trigger.schedule": "定时触发",
  "run.trigger.manual": "手动触发",
  "run.openSession": "Session {id}",
  "run.markRead": "标记已处理",
  "loading": "正在加载自动化…",
  "error.title": "无法加载自动化",
  "error.retry": "重试",
  "error.action": "操作失败，请重试。",
  "error.offline": "无法连接本机自动化服务，请确认 DSH 仍在运行后重试。",
  "time.now": "刚刚",
  "time.minuteAgo": "{count} 分钟前",
  "time.hourAgo": "{count} 小时前",
  "time.dayAgo": "{count} 天前",
  "time.inMinute": "{count} 分钟后",
  "time.inHour": "{count} 小时后",
  "time.inDay": "{count} 天后",
  "list.title": "定时任务",
  "list.subtitle": "让 Agent 安排任务、设置提醒或监测更新",
  "list.searchPlaceholder": "搜索已安排任务",
  "list.clearSearch": "清除搜索",
  "list.noMatch": "没有匹配的任务",
  "list.clearFilter": "清除筛选",
  "filter.label": "按状态筛选",
  "filter.all": "全部",
  "filter.active": "已开启",
  "filter.paused": "已暂停",
  "filter.finished": "已完成",
  "detail.title": "任务详情",
  "detail.close": "关闭详情",
  "detail.detailGroup": "详情",
  "detail.scheduleGroup": "频率",
  "detail.runtime": "运行时",
  "detail.runtimeValue": "每次运行新建聊天",
  "detail.runtimeHint": "定时运行不会继承当前对话的上下文。",
  "detail.defaultModel": "默认模型",
  "detail.modelSearch": "搜索模型",
  "detail.advanced": "高级",
  "detail.save": "保存",
  "detail.saving": "保存中…",
  "detail.unsaved": "有未保存的修改",
  "detail.promptExpand": "展开全部",
  "detail.promptCollapse": "收起",
  "detail.discard": "放弃未保存的修改？",
  "detail.discardHint": "关闭或切换任务后，尚未保存的修改会丢失。",
  "detail.discardConfirm": "放弃修改",
  "detail.keepEditing": "继续编辑",
  "history.title": "运行历史记录",
  "history.expand": "查看全部 {count} 条",
  "history.collapse": "收起",
  "history.empty": "暂无运行记录。",
}
