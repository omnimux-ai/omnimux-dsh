...
export function DraftWorkspace({ mode }) {
  ...
  const slots = mode === 'image' ? [['reference', '参考图', 'image']] : draft.submode === 'firstlast' ? [['first', '首帧', 'image'], ['last', '尾帧', 'image']] : draft.submode === 'ref' ? [['reference-video', '参考视频', 'video'], ['reference-image', '参考图', 'image']] : [['edit-video', '编辑视频', 'video'], ['edit-image', '编辑参考图', 'image'], ['edit-portrait', '人像参考', 'image']]
  ...
  return <section className="studio-draft-card" aria-label={`${mode} 草稿`}>
    ...
    {mode === 'video' && draft.submode === 'firstlast' && <Button onClick={() => patch({ references: draft.references.map(ref => ({ ...ref, slot: ref.slot === 'first' ? 'last' : ref.slot === 'last' ? 'first' : ref.slot })) })}>交换首尾帧</Button>}
    ...
    {draft.spec && <div className="studio-parameters">
      ...
      <div className="studio-parameter-options">
