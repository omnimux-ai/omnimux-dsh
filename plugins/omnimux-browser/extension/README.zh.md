...
在 TikTok 页面上，扩展会在左侧栏底部头像上方加一个自己的入口，提供三个快捷操作：**下载无水印视频**、**下载原视频音频**、**保存到灵感库**。

这些动作由本机 OmniMux 主程序执行，而不是在页面里完成：解析直链、写入文件、写入灵感库记录都由主程序负责。所以下载到的是真正的源文件，而不是页面内的一次性地址；保存的作品在入库完成的当下就能在灵感库里看到。

快捷操作只作用于「当前页面正在展示的那条作品」或「指针正指向的那条作品」。搜索页、探索页这类没有指向任何作品的页面，会直接说明「没有可操作的作品」，不会替你挑一条。

有两个前提：OmniMux 主程序需要处于运行状态（没运行时入口会直接说明），TikTok 需要已登录——只有能正常观看该作品的会话才能解析出无水印直链。任何一步走不通，都会在菜单那一行写出具体原因，不会静默失败。
## 架构
