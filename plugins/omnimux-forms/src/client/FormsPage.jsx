...
function Example({ example }) {
  return <Box as="figure" margin="0"><Box aspectRatio="16/9" width="100%" overflow="hidden" borderRadius="10px" bg="var(--dsw-alias-bg-layer-2)">{example.kind === 'video' ? <video src={example.ref} controls preload="metadata" aria-label={example.title} style={{ width: '100%', height: '100%', objectFit: 'contain' }} /> : <img src={example.ref} alt={example.title} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />}</Box><Text as="figcaption" marginTop="10px" fontSize="13px" color="var(--dsw-alias-label-secondary)">{example.title}</Text></Box>
}
export function Editor({ definition, workspace, hub, onBack }) {
  ...
  useEffect(() => {
    ...
    root?.querySelector('input,textarea,button,[tabindex="0"]')?.focus()
  }, [state.errors])
  return <Flex direction="column" height="100%" minHeight="0">
    ...
    <Box overflowY="auto" flex="1" minHeight="0" padding={{ base: '20px 16px', md: '28px' }}>
      <Box maxWidth="1120px" marginInline="auto"><Heading fontFamily="inherit" fontSize="24px" fontWeight="600" margin="0 0 8px">{definition.title}</Heading><Text fontSize="14px" color="var(--dsw-alias-label-secondary)" margin="0 0 28px">{definition.description}</Text>
