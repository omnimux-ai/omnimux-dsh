...
await build({stdin:{contents:`import React from 'react';import {renderToStaticMarkup} from 'react-dom/server';import {ChakraProvider} from '@chakra-ui/react';import {FormField} from './src/client/Fields.jsx';import {formSystem} from './src/client/theme.js';export function render(field){return renderToStaticMarkup(<ChakraProvider value={formSystem}><FormField field={field} value={field.default} errors={[]} state={{staleRefs:[],uploads:[]}} editor={{set(){}}} hub={{}} workspaceId="w"/></ChakraProvider>)};export {formSystem};`,resolveDir:root,loader:'jsx'},bundle:true,format:'cjs',platform:'node',outfile,define:{'process.env.NODE_ENV':'"production"'}})
const {render,formSystem}=require(outfile)
