...
export type SuitePartStatus = 'installed' | 'already' | 'failed'
export type SuiteRuleTarget = 'project' | 'global'

export interface SuiteEntry {
  name: string
  title: string
  desc: string
  /** 技能项可选：技能在包内的相对路径。空串 = 沿用 `<包根>/skills/<name>/`。 */
  path?: string
}
...
export interface SuitePartItem {
  name: string
  status: SuitePartStatus | 'absent' | 'removed' | 'skipped'
  path?: string
  error?: string
}

export interface SuiteFailure {
  part: 'pack' | 'skills' | 'rules' | 'agents'
  name: string
  error: string
}
...
export interface SuiteInstallResult {
  ...
  skill: string
  title: string
  /** 三样都没新写入时才为 true（重复安装的 already 语义）。 */
  already: boolean
  /** 任一环节失败即 true；`failed` 列出未完成项。 */
  partial: boolean
  failed: SuiteFailure[]
  pack: { skill: string, dir: string, already: boolean }
  skills: { dir: string, total: number, installed: number, already: number, failed: number, items: SuitePartItem[] }
  rules: {
    target: SuiteRuleTarget
    file: string
    projectRoot: string
    total: number
    written: number
  ...
  agents: { dir: string, total: number, installed: number, already: number, failed: number, items: SuitePartItem[] }
}

export interface SuiteUninstallResult {
  id: string
  skills: { dir: string, removed: number, absent: number, items: SuitePartItem[] }
  rules: { items: (SuitePartItem & { target: SuiteRuleTarget, file: string })[] }
  agents: { dir: string, removed: number, absent: number, items: SuitePartItem[] }
}

export interface SuiteInstallOptions {
  /** 目录文档，用于复用 `installItem` 的 bundled / git 装载语义。 */
  catalog: { items: SuiteItem[] }
  item: SuiteItem
  home: string
  profileDir: string
  packageRoot: string
  ruleTarget?: SuiteRuleTarget
  /** 会话工作区目录；缺省回落到 `cwd`（再回落 `process.cwd()`）。 */
  projectDir?: string
  cwd?: string
}

export interface SuiteUninstallOptions {
  item: SuiteItem
  home: string
  ruleTarget?: SuiteRuleTarget
  projectDir?: string
 ...
 */
export function suiteRuleBegin(id: string): string {
  return `<!-- omnimux-suite:${id} begin -->`
}

export function suiteRuleEnd(id: string): string {
 ...
 */
export function spliceSuiteRules(text: string, id: string, body: string): string {
  ...
  if (start >= 0 && stop > start) {
    ...
    if (text.slice(start, regionEnd) === block) return text
 ...
 */
export function dropSuiteRules(text: string, id: string): string {
  const begin = suiteRuleBegin(id)
  const end = suiteRuleEnd(id)
  const start = text.indexOf(begin)
  const stop = text.indexOf(end)
...
export interface ProjectRootResolution {
  root: string
  /** 是否真的命中 `.git` 标记；未命中时按平台语义回落到起始目录。 */
  matched: boolean
}

/** 自 `startDir` 逐级上溯找项目根标记，与平台 `dsh-agent-instructions` 同规则。 */
export function findProjectRoot(startDir: string, markers: string[] = ['.git']): ProjectRootResolution {
  const start = resolve(startDir)
  let current = start
  for (;;) {
    for (const marker of markers) {
      if (existsSync(join(current, marker))) return { root: current, matched: true }
    }
    const parent = dirname(current)
    if (parent === current) return { root: start, matched: false }
    current = parent
  }
}
...
export function resolveRuleTarget(opts: {
  ...
  projectDir?: string
  cwd?: string
}): { target: SuiteRuleTarget, file: string, projectRoot: string } {
 ...
 */
function skillSourceDir(packDir: string, nestedRoot: string, name: string, relPath: string): string {
...
function fileNamesWithExt(root: string, ext: string): string[] {
  if (!existsSync(root) || !statSync(root).isDirectory()) return []
...
function union(primary: string[], extra: string[]): string[] {
...
function manifest(item: SuiteItem): SuiteManifest {
  const suite = item.suite || ({} as Partial<SuiteManifest>)
  return {
 ...
 */
export function installSuite(opts: SuiteInstallOptions): SuiteInstallResult {
  const { home, item } = opts
  if (!item) throw new Error('unknown suite')
  if (item.kind !== 'suite') throw new Error(`item ${item.id} is not a suite`)
  const slug = String(item.skill || '').trim()
  if (!slug) throw new Error(`item ${item.id} missing skill`)
  // 包名会进 $DSH_HOME/skills/<slug>/，先过路径闸再谈落盘
  if (!isSafeName(slug)) throw new Error(`item ${item.id} 包名非法: ${slug}`)
  ...
  const skillsBefore = new Set(
    preNames.filter((name) => isSafeName(name) && existsSync(join(skillDir(home, name), 'SKILL.md'))),
  )

  const result: SuiteInstallResult = {
    id: item.id,
    skill: slug,
  ...
  try {
    const installed = installItem({
      catalog: opts.catalog as unknown as Parameters<typeof installItem>[0]['catalog'],
      id: item.id,
      home,
...
function installSuiteSkills(
  home: string,
  packDir: string,
  plan: SuiteManifest,
  result: SuiteInstallResult,
  skillsBefore: Set<string>,
): void {
  ...
  const flat: { entry: SuitePartItem, relPath: string }[] = []
  const pending: SuitePartItem[] = []
  for (const name of names) {
    ...
    const entry: SuitePartItem = { name, status: 'installed', path }
    result.skills.items.push(entry)
    pending.push(entry)
    if (relPath) flat.push({ entry, relPath })
  }
  ...
  try {
    ...
    for (const entry of pending) {
      if (flat.some((item) => item.entry === entry)) continue
      entry.status = 'failed'
...
function installSuiteRules(
  home: string,
  packDir: string,
  item: SuiteItem,
  plan: SuiteManifest,
  result: SuiteInstallResult,
  opts: SuiteInstallOptions,
): void {
  ...
  let target: { target: SuiteRuleTarget, file: string, projectRoot: string }
  try {
  ...
  try {
    ...
    if (next === before) {
      for (const entry of result.rules.items) {
        if (entry.status !== 'failed') entry.status = 'already'
      }
    } else {
      mkdirSync(dirname(target.file), { recursive: true })
      writeFileSync(target.file, next, 'utf8')
      for (const entry of result.rules.items) {
        if (entry.status !== 'failed') entry.status = 'installed'
      }
    }
    for (const entry of result.rules.items) if (entry.status === 'installed' || entry.status === 'already') entry.path = target.file
  } catch (err) {
    const error = message(err)
    for (const entry of result.rules.items) {
      if (entry.status === 'failed') continue
...
function tally(items: SuitePartItem[]): { total: number, ok: number, already: number, failed: number } {
  return {
    total: items.length,
...
function applyRuleCounts(result: SuiteInstallResult): void {
 ...
 */
export async function uninstallSuite(opts: SuiteUninstallOptions): Promise<SuiteUninstallResult> {
  const { home, item } = opts
  if (!item || item.kind !== 'suite') throw new Error(`item ${item?.id || ''} is not a suite`)
  const slug = String(item.skill || '').trim()
  const plan = manifest(item)
  const packDir = slug ? skillDir(home, slug) : ''
  const skillsRoot = join(home, 'skills')
  const result: SuiteUninstallResult = {
    id: item.id,
  ...
  const names = union(
    [slug, ...plan.skills.map((entry) => entry.name)],
    nestedRoot ? dirNames(nestedRoot) : [],
  ).filter((name) => name && isSafeName(name))
  for (const name of names) {
    try {
  ...
  const targets: SuiteRuleTarget[] = opts.ruleTarget ? [opts.ruleTarget] : ['project', 'global']
  ...
  for (const name of agentNames) {
    if (!isSafeName(name) || !slug) continue
    const presetId = `${slug}-${name}`.toLowerCase().replace(/[^a-z0-9-]/g, '-')
    const dir = join(home, '.agent-presets', presetId)
    if (!existsSync(dir)) {
