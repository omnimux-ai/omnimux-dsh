...
import { checkLimit, failure, reject, resolveLimits, type ValidationLimits, type ValidationResult } from './install-validation-contract.js';
...
async function run(): Promise<void> {
  ...
  try {
    const data = workerData as { bytes: ArrayBuffer; format: 'zip' | 'markdown'; limits: ValidationLimits };
    if (data.format !== 'zip' && data.format !== 'markdown') reject('PACKAGE_FORMAT', 'INPUT_FORMAT');
    const limits = resolveLimits(data.limits);
    // Brand-check and measure the worker's actual ArrayBuffer before hashing or parsing.
    const byteLength: number = Object.getOwnPropertyDescriptor(ArrayBuffer.prototype, 'byteLength')!.get!.call(data.bytes);
    checkLimit(byteLength, data.format === 'zip' ? limits.zipBytes : limits.skillBytes, 'INPUT_BYTES');
    if (data.format === 'markdown') checkLimit(byteLength, limits.totalBytes, 'TOTAL_BYTES');
    ...
    const validated = data.format === 'zip' ? await validateZip(bytes, limits) : {
      ...
      manifest: [{ path: 'SKILL.md', kind: 'file' as const, bytes: bytes.length, sha256: contentHash }],
    };
