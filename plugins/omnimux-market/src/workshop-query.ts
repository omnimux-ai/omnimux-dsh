...
import type {
  AdmissionSummary, CatalogSkillItem, InstallOrigin, SourceStatus, WorkshopDomain, WorkshopInventoryInput,
  WorkshopQueryInput, WorkshopQueryRequest, WorkshopQueryResult, WorkshopSkill,
} from './types.js'

export type { AdmissionSummary } from './types.js'
 ...
 */
export function admitWorkshopCatalogSkill(item: CatalogSkillItem): boolean {
  if (!isOfficialShelfItem(item)) return true
...
/** 一次发现过程中门禁的原始结果，供 `summarizeAdmission` 收口成审计摘要。 */
export interface WorkshopAdmissionOutcome {
  /** 是否真的遇到过官方货架条目：无官方条目时为 false，便于区分「没跑」与「全过」。 */
  enforced: boolean
  rejectedIds: string[]
}

/** 汇总审计摘要：id 去重升序，最多保留 `ADMISSION_SKIPPED_ID_LIMIT` 条。 */
export function summarizeAdmission(outcome: WorkshopAdmissionOutcome): AdmissionSummary {
...
function warnAdmissionRejected(admission: AdmissionSummary, catalogRevision: string): void {
  const omitted = admission.skippedCount - admission.skippedIds.length
  console.warn(
    `[omnimux-market] 技能双语准入门禁拒绝了 ${admission.skippedCount} 条官方货架技能`
 ...
 * 等于门禁被绕过。
 */
function collectWorkshopDiscovery(
  input: WorkshopQueryInput, includeRemote: boolean,
): { skills: WorkshopSkill[]; admission: AdmissionSummary } {
  ...
  const outcome: WorkshopAdmissionOutcome = { enforced: false, rejectedIds: [] }
  ...
  if (includeRemote) {
    for (const row of input.remote) {
      ...
      if (!token || winners.has(token) || rejectedTokens.has(token)) continue
      const installed = inventory.get(token)
      winners.set(token, {
...
export function createWorkshopSnapshot(
  ...
  let admission: AdmissionSummary = summarizeAdmission({ enforced: false, rejectedIds: [] })
  ...
  items.sort(recent)
  if (admission.skippedCount > 0) warnAdmissionRejected(admission, input.catalogRevision)
