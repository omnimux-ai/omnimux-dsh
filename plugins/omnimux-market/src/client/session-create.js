    ...
    async function createSkillSession(opts = {}) {
      ...
      const promise = (async () => {
        try {
          ...
          const plazaTitle = typeof lookup === "function" ? (lookup("plaza.title") || "Skill工坊") : "Skill工坊";
          try {
            window.__omnimuxWorkbench?.open?.({ tabId: "omnimux-market:plaza", sessionId: sessionBId, title: typeof lookup === "function" ? lookup("plaza.title") : "Skill工坊" });
          } catch {}
