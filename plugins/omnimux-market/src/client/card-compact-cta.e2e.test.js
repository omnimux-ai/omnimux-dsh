  ...
  <div class="probe-card featured-card" style="width:320px">
    <div class="featured-cover-wrap">
      <div class="featured-hover-actions">
        <button class="hover-btn hover-btn-detail"><span class="${LABEL.slice(1)}">${detail}</span></button>
        <button class="hover-btn hover-btn-try"><span class="${LABEL.slice(1)}">${tryLabel}</span></button>
      </div>
    </div>
  </div>
</div>`
...
test('专家市场精选卡：卡片自身宽度驱动 CTA 退化，紧凑只留图标、充足显示文字且永不裁切', () => {
  ...
  for (const [locale, detail, tryLabel] of [
    ...
    const rows = probeWidths(detail, tryLabel, [140, 170, 200, 215, 260, 320, 420])
    const compact = rows.filter((r) => r.cardWidth <= 200)
    const roomy = rows.filter((r) => r.cardWidth >= 215)
...
test('专家市场精选卡：退化由具名容器查询判定，且声明落在卡片根节点上', () => {
  ...
  assert.match(MARKET_CSS, /@container market-featured-card \(max-width:208px\)/, '退化规则必须由具名容器查询判定')
})

test('专家市场精选卡：文字标签由源码渲染，点击行为保持原样', () => {
