...
function retireEntryId(name: string): string {
  return name.replace(/-\d{9,}$/, '')
}

function cleanupResidue(): void {
  for (const id of FIXTURE_IDS) rmSync(join(presetRoot(), id), { recursive: true, force: true })
  const retired = retiredRoot()
  if (!existsSync(retired)) return
  for (const name of readdirSync(retired)) {
    if (!RESIDUE_IDS.includes(retireEntryId(name))) continue
    rmSync(join(retired, name), { recursive: true, force: true })
  }
}

function writePresetFixture(id: string, meta: { name: string, description: string, order: number, avatar?: string }): void {
...
async function callApi(body: Record<string, unknown>): Promise<{ status: number, json: any }> {
...
interface PixelAvatarModule {
  generatePixelAvatarSvg: (seed?: unknown, options?: { size?: number }) => string
  generatePixelAvatarDataUrl: (seed?: unknown, options?: { size?: number }) => string
  isPixelAvatarDataUrl: (src: unknown) => boolean
  hashPixelAvatarSeed: (text: unknown) => number
  [key: string]: any
}
...
async function loadPixelAvatar(): Promise<PixelAvatarModule> {
  const url = new URL('../../src/client/plaza/pixel-avatar.js', import.meta.url).href
  return (await import(url)) as PixelAvatarModule
}

/** 调色板与像素几何数据在常量模块里，与生成逻辑分家。 */
async function loadPixelAvatarPalette(): Promise<Record<string, any>> {
  const url = new URL('../../src/client/plaza/pixel-avatar-constants.js', import.meta.url).href
  return (await import(url)) as Record<string, any>
}
...
test('STATIC_MARKET_EXPERTS puts the 6 built-ins ahead of the 8 market experts', () => {
  ...
  assert.deepEqual(
    STATIC_MARKET_EXPERTS.map((e) => e.id),
    [...BUILTIN_AGENT_PRESETS.map((p) => p.id), ...DEFAULT_MARKET_EXPERTS.map((e) => e.id)],
  )
})
...
test('built-in preset install never restores a stale .retired archive as a user preset', async () => {
  ...
  if (existedBefore) renameSync(builtinDir, stashDir)
  ...
  try {
    ...
    if (existedBefore) renameSync(stashDir, builtinDir)
  }
})
...
test('pixel avatar generator draws a full character (skin, hair, eyes, clothes, background)', async () => {
  ...
  for (let i = 0; i < 40; i += 1) {
    ...
    for (const match of svg.matchAll(/fill="(#[0-9a-f]{6})"/g)) usedColors.add(match[1])
  }
  assert.ok(usedColors.size >= 20, `像素头像用色过少（${usedColors.size}），特征丰富度不足`)

  const skinColors = new Set<string>(
    palette.PIXEL_AVATAR_SKIN_TONES.flatMap((tone: { base: string, shade: string }) => [tone.base, tone.shade]),
  )
