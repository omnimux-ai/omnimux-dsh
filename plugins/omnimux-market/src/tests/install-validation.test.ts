...
import { VALIDATION_LIMITS as L, checkLimit, type ValidationLimits, type ValidationResult } from '../install-validation-contract.js';
import { normalizePath, pathKey } from '../install-validation-path.js';
import { SKILL, extraField, unicodeExtra, zipFixture, type ZipFixtureEntry } from './install-validation-fixture.js';

function zip(bytes: Buffer, limits?: Partial<ValidationLimits>): Promise<ValidationResult> {
  return validateInstallPackage({ format: 'zip', fileName: 'sample.zip', bytes }, { limits });
}
function md(bytes = SKILL, limits?: Partial<ValidationLimits>): Promise<ValidationResult> {
  return validateInstallPackage({ format: 'markdown', fileName: 'SKILL.md', bytes }, { limits });
}
async function rejected(bytes: Buffer, code?: string): Promise<void> {
  const result = await zip(bytes);
  assert.equal(result.ok, false, JSON.stringify(result));
  if (!result.ok && code) assert.equal(result.code, code);
  assert(!('manifest' in result));
}
function packageWith(entry: ZipFixtureEntry): Buffer {
...
test('standalone Markdown returns only validated metadata and exact original byte hash', async () => {
  const result = await md(); assert(result.ok);
...
for (const method of [0, 8]) for (const descriptor of [undefined, 'signed', 'unsigned'] as const) {
  test(`normal ZIP method ${method}, descriptor ${descriptor ?? 'none'}`, async () => {
    ...
    assert.equal(result.manifest.length,2); assert.equal(result.totalBytes,SKILL.length+8);
  });
}
test('one common wrapper is stripped, directory and resource remain', async () => {
  ...
  assert(result.ok,JSON.stringify(result)); assert.deepEqual(result.manifest.map(x=>x.path),['SKILL.md','res','res/data']);
});
test('empty ordinary resource has a verified empty hash', async () => {
  const result=await zip(packageWith({name:'empty'})); assert(result.ok); assert.equal(result.manifest[1].sha256,createHash('sha256').digest('hex'));
});
test('synchronous copy isolates caller mutation and preserves caller buffer', async () => {
  const bytes=Buffer.from(SKILL); const original=createHash('sha256').update(bytes).digest('hex');
  const pending=md(bytes); bytes.fill(0); const result=await pending;
  assert(result.ok); assert.equal(result.contentHash,original); assert.equal(bytes.length,SKILL.length);
});
test('SharedArrayBuffer source is rejected before starting a worker', async () => {
  ...
  assert(!result.ok); assert.equal(result.reason,'UNSHARED_BYTES_REQUIRED');
});
test('a subarray hashes only its own range',async()=>{
  const all=Buffer.concat([Buffer.from('prefix'),SKILL,Buffer.from('suffix')]); const result=await md(all.subarray(6,6+SKILL.length));
  assert(result.ok); assert.equal(result.inputBytes,SKILL.length);
});
for(const name of ['skill.md','README.md','/tmp/SKILL.md']) test(`standalone name ${name} is rejected`,async()=>{
  const r=await validateInstallPackage({format:'markdown',fileName:name,bytes:SKILL}); assert(!r.ok);
});
for(const [label,text] of [
  ...
  ['multiple documents','---\nname: s\ndescription: d\n...\nname: other\n---\nbody'],
  ['unclosed','---\nname: s\ndescription: d\nbody'],
] as const) test(`Markdown rejects ${label}`,async()=>{const r=await md(Buffer.from(text)); assert(!r.ok,JSON.stringify(r));assert.equal(r.code,'INVALID_SKILL');});
for(const name of ['Upper','has space','../bad','a_b','-start','end-','two--hyphens','技能']) test(`invalid invocation name ${name}`,async()=>{
  const r=await md(Buffer.from(`---\nname: ${name}\ndescription: valid\n---\nbody`));assert(!r.ok);assert.equal(r.code,'INVALID_SKILL');
});
test('Unicode override cannot hide raw legacy traversal',async()=>{
  const raw=Buffer.concat([Buffer.from('../'),Buffer.from([0x82])]);await rejected(packageWith({name:raw,flags:0,extra:unicodeExtra(raw,'safe')}),'UNSAFE_PATH');
});
...
test('BOM and CRLF preserve original byte accounting',async()=>{
  ...
  const r=await md(bytes);assert(r.ok);assert.equal(r.inputBytes,bytes.length);
});
test('untrusted metadata cannot grant recommendation, permissions or auto update',async()=>{
  const r=await md(Buffer.from('---\nname: s\ndescription: d\nrecommended: true\nautoUpdate: true\npermissions: [all]\n---\n<script>never run</script>'));
  assert(r.ok);assert.deepEqual(r.metadata,{name:'s',description:'d',version:null});
});
...
test('fixed full folding covers C/F and non-Turkic behavior',()=>{
  assert.equal(pathKey('ßİΣς'), 'ssi\u0307σσ'); assert.notEqual(pathKey('I'),pathKey('ı'));
});
for(const mode of [0xa1ff0000,0x21ff0000,0x61ff0000,0x11ff0000,0xc1ff0000]) test(`reject special mode ${mode.toString(16)}`,()=>rejected(packageWith({name:'special',attrs:mode}),'UNSAFE_PATH'));
...
test('CP437 raw filename decodes losslessly',async()=>{
  const r=await zip(packageWith({name:Buffer.from([0x82,0x2e,0x74,0x78,0x74]),flags:0}));assert(r.ok,JSON.stringify(r)); assert.equal(r.manifest[1].path,'é.txt');
});
test('valid Unicode path extra with CP437 is supported',async()=>{
  const raw=Buffer.from([0x82,0x2e,0x74,0x78,0x74]);const r=await zip(packageWith({name:raw,flags:0,extra:unicodeExtra(raw,'é.txt')}));assert(r.ok,JSON.stringify(r));
});
test('bad Unicode extra CRC rejected',async()=>{
  const raw=Buffer.from([0x82]);const extra=unicodeExtra(raw,'é');extra[5]^=1;await rejected(packageWith({name:raw,flags:0,extra}),'UNSAFE_PATH');
});
...
for(const flags of [1,0x40,0x2000,0x10]) test(`reject flags ${flags}`,()=>rejected(packageWith({name:'x',flags})));
...
for(const descriptor of ['signed','unsigned'] as const) test(`descriptor ${descriptor} CRC mismatch rejected`,async()=>{
  const bytes=zipFixture([{name:'SKILL.md',bytes:SKILL,descriptor}]);bytes[30+8+SKILL.length+(descriptor==='signed'?4:0)]^=1;await rejected(bytes);
});
...
for(const [label,mutate] of [
  ['multi disk',(b:Buffer)=>b.writeUInt16LE(1,b.length-18)],
  ['ZIP64 sentinel',(b:Buffer)=>b.writeUInt16LE(65535,b.length-12)],
  ['central size',(b:Buffer)=>b.writeUInt32LE(1,b.length-10)],
  ['entry disk',(b:Buffer)=>b.writeUInt16LE(1,b.readUInt32LE(b.length-6)+34)],
  ['overlap',(b:Buffer)=>{const c=b.readUInt32LE(b.length-6);b.writeUInt32LE(0,c+46+8+42);}],
] as const) test(`structure rejects ${label}`,async()=>{const b=packageWith({name:'x'});mutate(b);await rejected(b);});
...
test('ZIP 20MiB and +1 use inclusive bounded input',async()=>{
  const at=await zip(Buffer.alloc(L.zipBytes));assert(!at.ok);assert.notEqual(at.reason,'INPUT_BYTES');
  const over=await zip(Buffer.alloc(L.zipBytes+1));assert(!over.ok);assert.equal(over.reason,'INPUT_BYTES');
});
test('standalone and ZIP SKILL bytes exact 1MiB accepted, +1 rejected',async()=>{
  ...
  assert((await md(exact)).ok);assert((await zip(zipFixture([{name:'SKILL.md',bytes:exact}]))).ok);
...
test('resource exact 10MiB accepted and +1 rejected',async()=>{
  const data=Buffer.alloc(L.resourceBytes,7);assert((await zip(packageWith({name:'data',bytes:data}))).ok);
...
test('entry count includes directory entries, 1000 accepted and 1001 rejected',async()=>{
  const entries:ZipFixtureEntry[]=[{name:'SKILL.md',bytes:SKILL},...Array.from({length:999},(_,i)=>({name:`d${i}/`}))];
  assert((await zip(zipFixture(entries))).ok);await rejected(zipFixture([...entries,{name:'extra/'}]),'PACKAGE_LIMIT');
});
test('raw frontmatter byte limit includes delimiter and CRLF, excludes BOM',async()=>{
  ...
  const r=await md(Buffer.from('\ufeff'+front.replace('#','#a')+'body'));assert(!r.ok);assert.equal(r.reason,'FRONTMATTER_BYTES');
});
...
test('240 code points accepted, 241 rejected',async()=>{
  assert((await zip(packageWith({name:'a'.repeat(240)}))).ok);await rejected(packageWith({name:'a'.repeat(241)}),'PACKAGE_LIMIT');
});
test('255 component bytes accepted, 256 rejected',async()=>{
  assert((await zip(packageWith({name:'é'.repeat(127)+'a'}))).ok);await rejected(packageWith({name:'é'.repeat(128)}),'PACKAGE_LIMIT');
});
test('UTF8 total bytes use actual encoding, not UTF16 length',()=>{
  const name=Array.from({length:4},()=> '😀'.repeat(59)).join('/');assert.equal(Array.from(name).length,239);
...
test('compression budget integer boundary and zero denominator',()=>{
  ...
  assert.doesNotThrow(()=>checkLimit(0,0,'COMPRESSION_RATIO'));assert.throws(()=>checkLimit(1,0,'COMPRESSION_RATIO'));
});
...
test('total bytes exact reduced budget accepted and +1 rejected during streaming',async()=>{
  ...
  assert((await zip(b,{totalBytes:SKILL.length+100})).ok);const r=await zip(b,{totalBytes:SKILL.length+99});assert(!r.ok);assert.equal(r.reason,'TOTAL_BYTES');
});
test('lowered actual output budget stops a lying deflate declaration',async()=>{
  const r=await zip(packageWith({name:'r',bytes:randomBytes(10000),method:8,declaredSize:10}),{resourceBytes:100});assert(!r.ok);assert.equal(r.reason,'RESOURCE_BYTES');
});
test('callers cannot raise any production hard limit',async()=>{
  for(const key of Object.keys(L) as (keyof ValidationLimits)[]) {const r=await md(SKILL,{[key]:L[key]+1});assert(!r.ok);assert.equal(r.reason,'INVALID_BUDGET');}
});
test('compressed empty directories are measured without contributing ratio budget',async()=>{
  ...
  assert(r.ok,JSON.stringify(r));assert.equal(r.totalBytes,SKILL.length);
...
test('real total 100MiB accepted and cumulative +1 rejected',async()=>{
  ...
  for(let offset=0;offset<resource.length;offset+=block.length)block.copy(resource,offset,0,Math.min(block.length,resource.length-offset));
  const entries:ZipFixtureEntry[]=[{name:'SKILL.md',bytes:SKILL,method:8},...Array.from({length:9},(_,i)=>({name:`r${i}`,bytes:resource,method:8})),{name:'last',bytes:resource.subarray(0,resource.length-SKILL.length),method:8}];
  const exact=zipFixture(entries);assert(exact.length<L.zipBytes);const r=await zip(exact);assert(r.ok,JSON.stringify(r));assert.equal(r.totalBytes,L.totalBytes);
  entries[entries.length-1].bytes=resource.subarray(0,resource.length-SKILL.length+1);await rejected(zipFixture(entries),'PACKAGE_LIMIT');
});
...
test('independent MD total budget cannot be bypassed',async()=>{
  const r=await md(SKILL,{totalBytes:SKILL.length-1});assert(!r.ok);assert.equal(r.reason,'TOTAL_BYTES');
});
test('already aborted input creates no result manifest',async()=>{
  const c=new AbortController();c.abort();const r=await validateInstallPackage({format:'markdown',fileName:'SKILL.md',bytes:SKILL},{signal:c.signal});assert(!r.ok);assert.equal(r.code,'VALIDATION_ABORTED');
});
for(const mode of ['cancel','timeout'] as const) test(`worker ${mode} actually exits before promise settles`,async()=>{
  const workers=new Set<number>();const destroyed=new Set<number>();
  const hook=createHook({init(id,type){if(type==='WORKER')workers.add(id);},destroy(id){destroyed.add(id);}});hook.enable();
  try {
    const c=new AbortController();const p=validateInstallPackage({format:'zip',fileName:'test.zip',bytes:packageWith({name:'large',bytes:randomBytes(1024*1024),method:8})},{signal:c.signal,limits:{timeoutMs:mode==='timeout'?1:30000}});
    if(mode==='cancel')c.abort();const r=await p;assert(!r.ok);assert.equal(r.code,mode==='cancel'?'VALIDATION_ABORTED':'VALIDATION_TIMEOUT');
    await immediate();assert.equal(workers.size,1);for(const id of workers)assert(destroyed.has(id),'WORKER resource must be destroyed');
  } finally {hook.disable();}
});

for (const field of ['format', 'fileName', 'bytes'] as const) {
  for (const format of ['markdown', 'zip'] as const) test(`admission rejects ${format} ${field} accessor without invoking it`, async () => {
    ...
    assert(!result.ok); assert.equal(calls, 0);
...
test('admission does not execute reentrant or detaching byte getters', async () => {
  ...
  const bytes = new Uint8Array(backing); bytes.set(SKILL);
  let calls = 0;
  const input = { format: 'markdown' as const, fileName: 'SKILL.md', get bytes(): Uint8Array {
    calls++;
  ...
  const result = await validateInstallPackage(input);
  assert(!result.ok); assert.equal(calls, 0); assert.equal(backing.byteLength, SKILL.length);
});
test('input proxies are rejected without invoking descriptor or get traps', async () => {
  let traps = 0;
  const input = new Proxy({ format: 'markdown' as const, fileName: 'SKILL.md', bytes: SKILL }, {
    get() { traps++; throw new Error('PRIVATE_PROXY'); },
    getOwnPropertyDescriptor() { traps++; throw new Error('PRIVATE_PROXY'); },
  });
  const result = await validateInstallPackage(input);
  assert(!result.ok); assert.equal(traps, 0);
});
test('inherited byte accessors are rejected without prototype traversal', async () => {
  ...
  assert(!(await validateInstallPackage(input)).ok); assert.equal(calls, 0);
});
for (const kind of ['buffer', 'uint8array', 'subclass'] as const) test(`intrinsic range preserves ${kind} subview hash without public getters`, async () => {
  ...
  assert(result.ok, JSON.stringify(result)); assert.equal(calls, 0);
...
test('shadowed SAB backing is rejected without executing public getters', async () => {
  const bytes = new Uint8Array(new SharedArrayBuffer(SKILL.length)); bytes.set(SKILL);
  let calls = 0;
  ...
  assert(!result.ok); assert.equal(result.reason, 'UNSHARED_BYTES_REQUIRED'); assert.equal(calls, 0);
});
test('detached ordinary backing returns a redacted failure before worker creation', async () => {
  const backing = new ArrayBuffer(SKILL.length); const bytes = new Uint8Array(backing); bytes.set(SKILL);
  structuredClone(backing, { transfer: [backing] });
  const workers = new Set<number>();
  const hook = createHook({ init(id, type) { if (type === 'WORKER') workers.add(id); } }); hook.enable();
  try {
    const result = await validateInstallPackage({ format: 'markdown', fileName: 'SKILL.md', bytes });
    assert(!result.ok); assert.deepEqual(Object.keys(result).sort(), ['code', 'ok', 'reason']); assert.equal(workers.size, 0);
  } finally { hook.disable(); }
});
test('real oversized ZIP cannot hide its intrinsic size or start a worker', async () => {
  const entries: ZipFixtureEntry[] = [{ name: 'SKILL.md', bytes: SKILL }, { name: 'a' }, { name: 'b' }];
  ...
  const bytes = zipFixture(entries); assert.equal(bytes.length, L.zipBytes + 1);
  let calls = 0;
  ...
  try {
    ...
    assert.equal(calls, 0); assert.equal(workers.size, 0);
  } finally { hook.disable(); }
});

/** Exercise the worker boundary directly, without the public admission checks. */
async function validateWorkerInput(bytes: ArrayBuffer | SharedArrayBuffer, format: string, limits: Partial<ValidationLimits> = {}): Promise<ValidationResult> {
  ...
  return new Promise<ValidationResult>((resolve, reject) => {
    ...
    worker.once('message', (message: ValidationResult) => { result = message; });
    worker.once('error', reject);
    worker.once('exit', code => {
      clearTimeout(timer);
      if (code !== 0 || !result) reject(new Error('worker did not return a result'));
      else resolve(result);
    });
  });
}
for (const format of ['zip', 'markdown'] as const) test(`worker independently checks actual ${format} input bytes`, async () => {
...
for (const budget of ['skillBytes', 'totalBytes'] as const) test(`worker independently checks reduced Markdown ${budget}`, async () => {
  const backing = new ArrayBuffer(SKILL.length); new Uint8Array(backing).set(SKILL);
...
test('worker independently enforces reduced ZIP budget', async () => {
  ...
  const backing = new ArrayBuffer(bytes.length); new Uint8Array(backing).set(bytes);
...
test('worker rejects shared backing and invalid formats with finite failures', async () => {
  for (const [bytes, format] of [[new SharedArrayBuffer(32), 'markdown'], [new ArrayBuffer(32), 'other']] as const) {
    const result = await validateWorkerInput(bytes, format);
    assert(!result.ok); assert.equal(result.code, 'PACKAGE_FORMAT');
