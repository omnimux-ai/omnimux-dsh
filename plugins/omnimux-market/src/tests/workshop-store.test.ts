...
import { emptyWorkshopState, compareAndSwapWorkshopState, parseWorkshopState, WorkshopStore,
  readWorkshopFile, validSourceRef } from '../workshop-store.js'
...
] as const) test(`state rejects ${name} without repair`, () => {
...
test('metadata read rejects links, escaping paths and bounded files', async () => {
  ...
  try {
    ...
    for (const name of ['../data', '/data', 'a\\b', 'a//b']) await assert.rejects(readWorkshopFile(root, name, 10), /UNSAFE_PATH/)
  } finally { await rm(root, { recursive: true, force: true }) }
})
