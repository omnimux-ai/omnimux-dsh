...
import {
  createWorkshopSnapshot, pageWorkshopSnapshot, retainWorkshopSnapshot,
  workshopDate, workshopDomains, workshopQueryKey, workshopSourceOptions,
  isWorkshopResponseApplicable,
...
const REQUEST: WorkshopQueryRequest = {
  view: 'discover', query: '', domain: 'all', source: 'all', uninstalledOnly: false, queryRevision: 7,
}
function catalogItem(token: string, extra: Partial<CatalogSkillItem> = {}): CatalogSkillItem {
  return {
...
function data(items: CatalogSkillItem[] = []): WorkshopQueryInput {
  return {
...
function installed(token: string, extra: Partial<WorkshopSkill> = {}): WorkshopSkill {
  return {
...
function versions(input: WorkshopQueryInput) {
...
function run(input: WorkshopQueryInput, change: Partial<WorkshopQueryRequest> = {}, size = 48, id = 'qa-snapshot') {
...
function remote(token: string) {
  const card = mapSkill({ slug: token, name: token, description: '动画 fixture' }, 'https://example.invalid')!
  return { card, sourceRef: { kind: 'skillhub' as const, identity: `owner/${token}`, version: null }, downloads: null, updatedAt: null, publishedAt: null }
}
...
test('QA domain membership preserves shared explicit-tag precedence and bounded fallback', () => {
  for (const value of [null, undefined, 0, '动画']) assert.deepEqual(workshopDomains(value), [])
...
test('QA request keys normalize case and outer whitespace but retain independent filters', () => {
  ...
  for (const change of [{ view: 'mine' as const }, { domain: '动画' as const }, { source: 'unknown' as const }, { uninstalledOnly: true }]) {
...
for (const [label, change] of Object.entries({
  ...
  test(`QA rejects invalid request: ${label}`, () => {
    assert.throws(() => run(data(), change as Partial<WorkshopQueryRequest>), { code: 'INVALID_REQUEST' })
  })
}
...
test('QA source failures, duplicate evidence and invalid fetched counts cannot assert exact', () => {
  ...
  for (const sourceStatus of [[], [...base.sourceStatus, base.sourceStatus[0]],
    ...
    base.sourceStatus.map((s) => ({ ...s, status: 'error' as const }))]) {
...
test('QA snapshot retention uses creation TTL and maximum eight across same-time insertions', () => {
  ...
  for (let i = 0; i < 10; i++) cache = retainWorkshopSnapshot(cache, run(input, {}, 1, `s-${i}`).snapshot, 100)
