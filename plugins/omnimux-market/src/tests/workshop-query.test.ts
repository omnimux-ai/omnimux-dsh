...
import {
  ADMISSION_SKIPPED_ID_LIMIT, WORKSHOP_CATEGORIES, WORKSHOP_DOMAINS, admitWorkshopCatalogSkill,
  createWorkshopSnapshot, pageWorkshopSnapshot, normalizeWorkshopDiscovery, summarizeAdmission,
  workshopDate, workshopDomains, workshopSourceOptions, retainWorkshopSnapshot, isWorkshopResponseApplicable,
...
import type { CatalogSkillItem, SkillCard, WorkshopQueryInput, WorkshopQueryRequest, WorkshopSkill } from '../types.js'

const request: WorkshopQueryRequest = { view: 'discover', query: '', domain: 'all', source: 'all', uninstalledOnly: false, queryRevision: 1 }
/** 官方货架条目默认双语齐备；需要验证门禁时用 `withoutBilingual()` 显式剥离字段。 */
function item(id: string, extra: Partial<CatalogSkillItem> = {}): CatalogSkillItem {
...
function withoutBilingual(row: CatalogSkillItem, fields: string[] = ['titleZh', 'titleEn', 'summaryZh', 'summaryEn']): CatalogSkillItem {
  const copy = { ...row } as unknown as Record<string, unknown>
  for (const field of fields) delete copy[field]
  return copy as unknown as CatalogSkillItem
}
function shelf(id: string, extra: Partial<CatalogSkillItem> = {}): CatalogSkillItem {
  return item(id, { recommended: true, tags: ['动画'], ...extra })
}
function input(items: CatalogSkillItem[] = []): WorkshopQueryInput {
...
function skill(id: string, extra: Partial<WorkshopSkill> = {}): WorkshopSkill {
...
function versions(data: WorkshopQueryInput) {
  return { scopeKey: data.inventory.scopeKey, catalogRevision: data.catalogRevision, inventoryRevision: data.inventory.revision }
}
function query(data: WorkshopQueryInput, change: Partial<WorkshopQueryRequest> = {}, pageSize = 48) {
...
test('AC-06–11: strict controlled recommendation, 0/1/multiple domains, no demo creation', () => {
  for (const recommended of [undefined, false, 'true', 1, {}, []]) {
    const data = input([item('a', { recommended } as Partial<CatalogSkillItem>)])
  ...
  for (const domain of ['all', 'featured', '动画', '商业广告'] as const) {
...
test('controlled recommendation requires an own strict true at parser and direct query entrypoints', () => {
  const inherited: CatalogSkillItem = Object.assign(Object.create({ recommended: true }), item('inherited'))
  const ownTrue: CatalogSkillItem = Object.assign(Object.create(null), item('own-true', { recommended: true }))
  const ownFalse: CatalogSkillItem = Object.assign(Object.create({ recommended: true }), item('own-false', { recommended: false }))
...
for (const inherited of [false, true]) {
  test(`remote slug ignores ${inherited ? 'inherited' : 'own'} skill extension without borrowing installed state`, () => {
    ...
    const extended: typeof card = inherited
...
test('AC-19/49: missing, partial, unexhausted and failed sources cannot claim exact', () => {
  for (const status of ['partial', 'error', 'complete'] as const) {
...
test('AC-20: cursors bind all filters, query revision, catalog, scope and inventory revisions', () => {
  ...
  for (const change of [{ query: 'a' }, { domain: '动画' as const }, { source: 'local' as const }, { view: 'mine' as const }, { uninstalledOnly: true }, { queryRevision: 2 }]) {
...
test('invalid filters/revisions/page sizes fail instead of broadening to all', () => {
  for (const change of [{ domain: 'unknown' }, { uninstalledOnly: 'false' }, { queryRevision: NaN }, { view: 'mine', domain: 'featured' }]) {
    assert.throws(() => query(input(), change as Partial<WorkshopQueryRequest>), /INVALID_REQUEST/)
  }
  for (const size of [0, 81, 1.5]) assert.throws(() => query(input(), {}, size), /INVALID_REQUEST/)
})

test('DATA-01: snapshot limit 8 and exact TTL boundary, duplicate IDs rejected, result copies isolated', () => {
  const data = input([item('a')])
  let cache: ReturnType<typeof createWorkshopSnapshot>[] = []
  for (let i = 0; i < 9; i++) cache = retainWorkshopSnapshot(cache, createWorkshopSnapshot(data, request, `s${i}`, i), i)
...
test('T02-02（B8 · ADR-BL-01）：被拒 token 不得被远程同名行顶替', () => {
  ...
  data.remote = [{
    card: { id: 'broken', slug: 'broken', name: '远端 broken', description: '远端同名行', category: '', categoryLabel: '',
      version: '', downloads: 0, stars: 0, installs: 0, pageUrl: '', channel: 'skillhub' } satisfies SkillCard,
...
test('T02-05（B10/B13）：无官方条目或目录不可用时 enforced 为 false，且老响应判定不受影响', () => {
  ...
  const legacySnapshot = { ...empty.snapshot } as Record<string, unknown>
  delete legacySnapshot.admission
  const paged = pageWorkshopSnapshot(legacySnapshot as unknown as typeof empty.snapshot, empty.req, legacyVersions, 0)
...
test('T02-08：summarizeAdmission 去重升序并按上限截断', () => {
  ...
  assert.deepEqual(summarizeAdmission({ enforced: false, rejectedIds: ['b', 'a', 'b'] }),
    { enforced: false, skippedCount: 2, skippedIds: ['a', 'b'] })
...
test('T02-09：运行时门禁只告警不外抛，脏数据不得让市场不可用', () => {
  ...
  try {
    ...
    const hostile = shelf('hostile', { titleZh: {}, titleEn: [], summaryZh: 1, summaryEn: null } as unknown as Partial<CatalogSkillItem>)
