...
import type { WorkshopReadAuthorization } from '../workshop-request-guard.js'
import type { WorkshopQueryRequest } from '../types.js'

const origin = 'http://127.0.0.1:44444'
function request(url = '/omnimux-market/workshop/workshopInventory', method = 'GET'): IncomingMessage {
  const req = new IncomingMessage(new Socket())
  req.url = url; req.method = method; req.headers.origin = origin; req.rawHeaders = ['Origin', origin]
  return req
}
function authorization(patch: Partial<WorkshopReadAuthorization> = {}): WorkshopReadAuthorization {
  return { connection: { requestRejection: () => undefined }, trustedOrigin: origin, authorizeRead: () => true, ...patch }
}
function response(req: IncomingMessage): { res: ServerResponse; body(): Record<string, unknown> } {
  const res = new ServerResponse(req)
  let text = ''
  res.end = ((chunk: unknown) => { text = String(chunk); return res }) as ServerResponse['end']
  return { res, body: () => JSON.parse(text) }
}
const query: WorkshopQueryRequest = { view: 'discover', query: '', domain: 'all', source: 'all', uninstalledOnly: false, queryRevision: 1 }
...
test('exact Origin compares protocol/host/effective port; malformed values fail closed', () => {
  ...
  for (const value of ['null', 'https://example.test/', 'https://u:p@example.test', 'https://example.test/path', ' https://example.test', 'file:///a', 'https://example.test?q=x']) assert.equal(canonicalWorkshopOrigin(value), null)
})
...
] as const) test(`guard ${name} rejects before body iterator`, async () => {
...
for (const bad of ['https://127.0.0.1:44444', 'http://127.0.0.1:44445', 'http://evil.test:44444', undefined]) test(`guard rejects Origin ${bad}`, async () => {
  const req = request(); req.headers.origin = bad; req.headers['x-forwarded-host'] = '127.0.0.1:44444'
...
test('guard rejects duplicate Origin, POST and GET bodies; legal read passes', async () => {
  ...
  const req = request(); await guard.authorizeHeaders(req, 'workshopInventory')
  ...
  const body = request(); body.headers['content-length'] = '20'
...
test('authorized public Host and actual local API consume real fixture files and catalog', async () => {
  ...
  try {
    ...
    assert.equal(detail.skill.title, 'Fixture'); assert.equal(detail.descriptionComplete, false)
    const caps = await host.workshopCapabilities(request())
    assert.equal(caps.writable, false); assert.equal(caps.unifiedPolicy, false)
...
test('default Host missing scope reports unavailable; no guessed cfg.skillsDir scan', async () => {
  ...
  assert.equal(inv.status, 'error'); assert(inv.reasons.includes('SCOPE_UNVERIFIED'))
...
test('API refuses unknown query keys and malformed JSON without body reads or private errors', async () => {
  ...
  for (const suffix of ['?path=/private', '?request=%7B', '?request=a&request=b']) {
    ...
    assert.equal(capture.res.statusCode, 400); assert.equal(capture.body().code, 'INVALID_REQUEST')
  }
})
