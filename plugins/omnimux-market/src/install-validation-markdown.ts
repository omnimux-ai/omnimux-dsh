...
import { checkLimit, reject, type SkillMetadata, type ValidationLimits } from './install-validation-contract.js';

/** Decode without replacement characters; a single leading BOM is handled by the caller. */
export function strictUtf8(bytes: Uint8Array, code: 'INVALID_SKILL' | 'UNSAFE_PATH'): string {
  try {
...
export function validateMarkdown(bytes: Buffer, limits: ValidationLimits): SkillMetadata {
  ...
  while (cursor <= bytes.length) {
    ...
    if (end - start > limits.frontmatterBytes) reject('PACKAGE_LIMIT', 'FRONTMATTER_BYTES');
    if (newline === -1) break;
    cursor = newline + 1;
  }
  if (closeEnd < 0) reject('INVALID_SKILL', 'FRONTMATTER_UNCLOSED');
  checkLimit(closeEnd - start, limits.frontmatterBytes, 'FRONTMATTER_BYTES');
  if (!strictUtf8(bytes.subarray(closeEnd), 'INVALID_SKILL').trim()) reject('INVALID_SKILL', 'EMPTY_BODY');
  ...
  if (documents.length !== 1) reject('INVALID_SKILL', 'MULTIPLE_YAML_DOCUMENTS');
  ...
  while (pending.length) {
    ...
    if (isAlias(node)) reject('INVALID_SKILL', 'YAML_ALIAS');
    if (isMap(node) || isSeq(node) || isScalar(node)) {
      if (node.tag) reject('INVALID_SKILL', 'YAML_TAG');
      if (isMap(node)) {
        for (const item of node.items) {
          if (!isScalar(item.key) || typeof item.key.value !== 'string') reject('INVALID_SKILL', 'YAML_KEY');
          pending.push(item.key, item.value);
        }
      } else if (isSeq(node)) pending.push(...node.items);
    }
  }
  const name: unknown = document.get('name');
  const description: unknown = document.get('description');
  const version: unknown = document.get('version');
