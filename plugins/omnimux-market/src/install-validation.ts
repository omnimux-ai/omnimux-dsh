...
import { checkLimit, failure, reject, resolveLimits, type ValidationInput, type ValidationOptions, type ValidationResult } from './install-validation-contract.js';
export type { ValidationInput, ValidationOptions, ValidationResult, ValidatedPackage } from './install-validation-contract.js';
...
export function validateInstallPackage(input: ValidationInput, options: ValidationOptions = {}): Promise<ValidationResult> {
  try {
    const signal = options.signal;
    const limits = resolveLimits(options.limits);
    if (signal?.aborted) return Promise.resolve({ ok: false, code: 'VALIDATION_ABORTED', reason: 'ABORTED' });
    if (!input || types.isProxy(input)) reject('PACKAGE_FORMAT', 'INPUT_FORMAT');
    ...
    if (!formatField || !('value' in formatField) || !nameField || !('value' in nameField)) reject('PACKAGE_FORMAT', 'INPUT_FORMAT');
    const format: unknown = formatField.value;
    const fileName: unknown = nameField.value;
    if ((format !== 'zip' && format !== 'markdown') || typeof fileName !== 'string') reject('PACKAGE_FORMAT', 'INPUT_FORMAT');
    if (format === 'markdown' ? fileName !== 'SKILL.md' : !/\.zip$/i.test(fileName)) reject('PACKAGE_FORMAT', 'FILE_NAME');
    ...
    const bytes: unknown = bytesField.value;
    if (!types.isUint8Array(bytes)) reject('PACKAGE_FORMAT', 'UNSHARED_BYTES_REQUIRED');
    const backing: unknown = bufferGetter.call(bytes);
    if (!types.isArrayBuffer(backing)) reject('PACKAGE_FORMAT', 'UNSHARED_BYTES_REQUIRED');
    const byteOffset: number = offsetGetter.call(bytes);
    const byteLength: number = lengthGetter.call(bytes);
    ...
    return new Promise<ValidationResult>(resolve => {
      ...
      let stopping = false;
      const stop = (next: ValidationResult): void => {
        if (stopping) return;
        stopping = true;
        result = next;
      ...
      const abort = (): void => stop({ ok: false, code: 'VALIDATION_ABORTED', reason: 'ABORTED' });
      const timeout = (): void => stop({ ok: false, code: 'VALIDATION_TIMEOUT', reason: 'DEADLINE' });
      const timer = setTimeout(timeout, Math.max(0, limits.timeoutMs - (performance.now() - started)));
      worker.on('message', (message: ValidationResult) => {
        if (performance.now() - started >= limits.timeoutMs) timeout();
        else stop(message);
      });
