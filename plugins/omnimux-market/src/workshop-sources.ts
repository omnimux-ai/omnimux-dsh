...
import { createWorkshopSnapshot, normalizeWorkshopDiscovery, pageWorkshopSnapshot, retainWorkshopSnapshot,
  workshopQueryKey, WorkshopQueryError, WORKSHOP_QUERY_LIMITS } from './workshop-query.js'
import type { WorkshopQuerySnapshot } from './workshop-query.js'
import { readWorkshopFile, validSourceRef, WorkshopReadError } from './workshop-store.js'
import type { CatalogDoc, PluginConfig, SourceRef, SourceStatus, SkillHubSkillRaw,
  WorkshopInventoryResult, WorkshopQueryInput, WorkshopQueryRequest, WorkshopQueryResult, WorkshopRemoteCandidate, WorkshopSkill } from './types.js'
...
export interface WorkshopRemotePage {
  rows: WorkshopRemoteCandidate[]
  nextCursor: string | null
  exhausted: boolean
  /** Stable source snapshots must be independently supported, not inferred from total. */
  stable: boolean
}
export interface WorkshopSources {
  catalog(): Promise<WorkshopCatalogResult>
  remotePage(query: string, cursor: string | null, signal: AbortSignal): Promise<WorkshopRemotePage>
}

/** Only the packaged controlled catalog; no user marketplace or installed-file probing. */
export async function readWorkshopCatalog(root = packageRoot()): Promise<WorkshopCatalogResult> {
  try {
    ...
    return { catalog, revision: createHash('sha256').update(bytes).digest('hex'),
      sourceStatus: (['omnimux', 'workbuddy'] as const).map((origin) => ({ origin, status: 'complete', exhausted: true,
        fetched: catalog.items.filter((r) => catalogSkillChannel(r) === (origin === 'omnimux' ? 'custom' : 'workbuddy')).length })) }
  } catch {
    return { catalog: { items: [] }, revision: 'catalog-unavailable', sourceStatus: (['omnimux', 'workbuddy'] as const)
...
async function remoteJson(url: string, userAgent: string, signal: AbortSignal, fetchImpl: typeof fetch): Promise<unknown> {
  signal.throwIfAborted()
  const response = await fetchImpl(url, { signal, redirect: 'error', headers: { accept: 'application/json', 'user-agent': userAgent } })
  if (!response.ok || !response.body) { await response.body?.cancel(); throw new WorkshopReadError('SOURCE_ERROR') }
  ...
  try {
    while (true) {
      ...
      if (size > 2 * 1024 * 1024) throw new WorkshopReadError('SOURCE_BODY_LIMIT')
...
export function createWorkshopSources(cfg: PluginConfig, fetchImpl: typeof fetch = fetch): WorkshopSources {
  return {
    catalog: () => readWorkshopCatalog(),
    async remotePage(query, cursor, signal) {
      if (!query.trim()) throw new WorkshopReadError('INVALID_REQUEST', 400)
      const page = cursor === null ? 1 : Number(cursor)
      if (!Number.isSafeInteger(page) || page < 1 || page > 20) throw new WorkshopReadError('INVALID_REQUEST', 400)
      const base = new URL(cfg.apiBase)
      if (!['https:', 'http:'].includes(base.protocol) || base.username || base.password || base.search || base.hash) throw new WorkshopReadError('SOURCE_CONFIG_INVALID')
      ...
      const raw = await remoteJson(url.href, cfg.userAgent, signal, fetchImpl) as { code?: unknown; data?: { skills?: unknown; total?: unknown } }
      if (raw?.code !== 0 || !raw.data || !Array.isArray(raw.data.skills)) throw new WorkshopReadError('SOURCE_INVALID')
      const rows = raw.data.skills.map((value: unknown): WorkshopRemoteCandidate => {
        if (!value || typeof value !== 'object' || Array.isArray(value)) throw new WorkshopReadError('SOURCE_INVALID')
        const item = value as SkillHubSkillRaw
        if (typeof item.slug !== 'string' || !/^[a-z0-9][a-z0-9_-]{0,127}$/i.test(item.slug)
          || (item.name !== undefined && typeof item.name !== 'string')
          || (item.description !== undefined && typeof item.description !== 'string')) throw new WorkshopReadError('SOURCE_INVALID')
        const card = mapSkill(item, cfg.webBase)
        if (!card) throw new WorkshopReadError('SOURCE_INVALID')
...
export async function enumerateWorkshopRemote(sources: WorkshopSources, query: string, options: {
  signal?: AbortSignal; now?: () => number; timeoutMs?: number
} = {}): Promise<{ remote: WorkshopRemoteCandidate[]; status: SourceStatus }> {
  const now = options.now || (() => performance.now())
  const timeoutMs = options.timeoutMs ?? WORKSHOP_QUERY_LIMITS.wallMs
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0 || timeoutMs > WORKSHOP_QUERY_LIMITS.wallMs) throw new WorkshopReadError('INVALID_REQUEST', 400)
  ...
  const remote: WorkshopRemoteCandidate[] = []
  let cursor: string | null = null, stable = true, code = ''
  try {
    while (canLoadWorkshopPage(budget, now())) {
      controller.signal.throwIfAborted()
      // Race is needed for a provider that fails to settle after cancellation.
      const page: WorkshopRemotePage = await abortable(sources.remotePage(query, cursor, controller.signal), controller.signal)
    ...
    if (!budget.stop) code = 'SOURCE_TIMEOUT'
    else if (budget.stop !== 'complete') code = `SOURCE_${budget.stop.toUpperCase().replace('-', '_')}`
    else if (!stable) code = 'SOURCE_SNAPSHOT_UNVERIFIED'
  } catch (error) {
...
export interface WorkshopDetailRequest { skillKey: string; sourceRef: SourceRef | null; installId?: string }

/** One service per authorized scope; never accept roots or a caller-selected scope. */
export class QueryService {
  private snapshots: WorkshopQuerySnapshot[] = []
  constructor(private readonly sources: WorkshopSources,
    private readonly inventory: { reconcile(): Promise<WorkshopInventoryResult> },
    private readonly now: () => number = () => performance.now()) {}

  async query(request: WorkshopQueryRequest): Promise<WorkshopQueryResult> {
    workshopQueryKey(request)
    if (request.query.length > 2048 || (request.cursor?.length || 0) > 2048) throw new WorkshopQueryError('INVALID_REQUEST')
    const inventory = await this.inventory.reconcile()
    if (inventory.status === 'error' || (request.view === 'discover' && inventory.status !== 'complete')) throw new WorkshopQueryError('INVENTORY_UNAVAILABLE')
    ...
    if (request.cursor) {
      let id: unknown
      try { id = JSON.parse(request.cursor)[1] } catch { throw new WorkshopQueryError('CURSOR_EXPIRED') }
      const snapshot = this.snapshots.find((s) => s.id === id)
      if (!snapshot) throw new WorkshopQueryError('CURSOR_EXPIRED')
    ...
    if (current.revision !== inventory.revision || currentCatalog.revision !== catalog.revision) throw new WorkshopQueryError('CURSOR_EXPIRED')
    const input: WorkshopQueryInput = { catalog: catalog.catalog, catalogRevision: catalog.revision,
  ...
  async detail(request: WorkshopDetailRequest): Promise<{ skill: WorkshopSkill; descriptionComplete: boolean; reasons: string[] }> {
    if (!request || typeof request.skillKey !== 'string' || (request.sourceRef !== null && !validSourceRef(request.sourceRef))) throw new WorkshopReadError('INVALID_REQUEST', 400)
    const inventory = await this.inventory.reconcile()
    ...
    if (request.installId) {
      ...
      if (!record || !sameSource(record.skill.sourceRef, request.sourceRef)) throw new WorkshopReadError('SOURCE_CHANGED')
    ...
    if (request.sourceRef?.kind === 'catalog') {
      if (request.sourceRef.revision !== catalog.revision) throw new WorkshopReadError('SOURCE_CHANGED')
      const item = catalog.catalog.items.find((row) => row.id === (request.sourceRef as Extract<SourceRef, { kind: 'catalog' }>).catalogId)
      if (!item) throw new WorkshopReadError('SOURCE_UNAVAILABLE', 404)
      ...
      if (!skill || skill.skillKey !== request.skillKey) throw new WorkshopReadError('SOURCE_CHANGED')
    ...
    for (const snapshot of this.snapshots) {
      if (this.now() - snapshot.createdAt >= WORKSHOP_QUERY_LIMITS.ttlMs || snapshot.inventoryRevision !== inventory.revision
        || snapshot.catalogRevision !== catalog.revision) continue
      ...
      if (skill) return { skill: structuredClone(skill), descriptionComplete: false, reasons: ['EXACT_DETAIL_PROVIDER_UNAVAILABLE'] }
    }
...
function sameSource(a: SourceRef | null, b: SourceRef | null): boolean {
  ...
  return Object.keys(a).every((key) => (a as unknown as Record<string, unknown>)[key] === (b as unknown as Record<string, unknown>)[key])
