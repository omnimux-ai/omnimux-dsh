...
## OmniMux runtime and scope

This packaged adaptation requires Python 3.10+ and a command-execution tool in the user's environment. OmniMux does not supply or install Python for this Skill. Check availability before an authorized run; missing dependencies must be reported, not treated as configured. Network and Woot endpoint availability are not guaranteed. No collection is triggered by installation or recommendation.

Resolve `SKILL_ROOT` to the absolute directory containing this installed SKILL.md using the installation result or loaded Skill resource path; do not assume a home/profile path. Run commands with the user's authorized project as the working directory. Every `scripts/` and `references/` resource is relative to `SKILL_ROOT`, while input/output paths remain relative to the project. Set `SKILL_ROOT` only from that resolved path before using the examples below.

Collect only authorized targets under applicable site rules and data rights; do not bypass access restrictions. The MIT software licenses do not grant rights to redistribute review content. This Skill collects review data, not a complete optimization report. Other BGGG suite Skills are optional and are not included or automatically invoked.
...
```bash
python3 "$SKILL_ROOT/scripts/run_batch.py" \
...
```bash
python3 "$SKILL_ROOT/scripts/normalize_reviews.py" \
  --run-dir work/amazon/2026-07-25 \
  --targets config/amazon_targets.tsv \
