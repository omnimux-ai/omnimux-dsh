...
description: 基于爆款参考视频或创作意图，运用专业导演与制片人心法，执行原片深度解构、黄金Hook提炼、字词级语义对齐、多模态素材集群调度、批量变体生成，并与 OmniMux 创作画布及 Clip 剪辑工作台无缝融合。
---

# 爆款视频全链路复刻技能（Viral Video Replication）
...
## OmniMux 基础设施与业务闭环契约（必遵）

在 OmniMux 桌面端与插件体系中，本技能作为“爆款视频复刻”的顶层执行中枢，严格遵循 5 步标准业务闭环（SOP），100% 依托现有设施，零新建孤岛界面：

1. **输入与意图承接（对话区）**：
   - 接收用户在会话中发送的抖音、快手、TikTok、YouTube Shorts 等视频链接或本地样片文件；
   - 明确用户的改写意图（如：“换成我的产品/脸/语言，批量派生变体”）。

2. **五维深度解构（智能体心法）**：
   - 提取**黄金前 3 秒钩子（Hook）**：制造冲突、悬念、共鸣或反直觉结论；
   - 解构镜头语言：A-roll 主播视线、构图偏置、手势起伏，与 B-roll 搞笑反差插图；
   - 输出三份标准交付物记录于项目中：
     - `Analysis`：原片整体结构、分镜清单、词级时间线；
     - `Brief`：用户商业目标、产品卖点、目标受众画像；
     - `Treatment`：改写后的创意方案、人物形象设定、语气与视觉风格。

3. **创作画布编排（OmniMux Workflow 画布）**：
   - 智能体直接调用画布节点投放接口，在创作画布上自动摆放拓扑结构：
     `【参考样片节点】→【剧本与分镜改写节点】→【多模态素材集群（Seedance/GPT/FishAudio）】→【视频合成超级节点】`；
   - 节点之间连线清晰，卡槽参数完备，用户可在画布上直观审查并调整。

4. **词级语义对齐与批量裂变（Hypit 编译内核）**：
   - 坚持“字词级锚定”原则：台词中的 `@anchor` 标签绑定画面插图、花字与音效；
   - 底层由 WhisperX 进行高精度声学时间戳对齐，换词、换语种时间轴自动重排；
   - 执行中枢调用多进程无头渲染管线，一键批量产出 100+ 变体工程；
   - 生成的角色肖像、独特音色与优质素材自动存入**资产库（omnimux-assets）**沉淀复用。

5. **多轨剪辑工作台精修（OmniMux Clip 剪辑器）**：
   - 产物通过标准通信协议（`OMNIMUX_CLIP_OPEN`）直接投递给剪辑工作台；
   - 剪辑工作台原生铺开专业多轨时间轴（**智能字幕轨、特写插图轨、主播画面轨、背景音效轨**）；
   - 用户可进行所见即所得的人工微调（调字号、切片缩放、加转场、精调音量），一键导出 4K 高清成片。

---

## 导演与制片人心法（Director & Producer Principles）
...
reference, WhisperX's transcript and word times connect speech with picture changes.
In OmniMux, generation models (Seedance, GPT-Image, Grok-Imagine, Fish Audio) are orchestrated
transparently through the OmniMux Execution Hub without requiring the user to configure external endpoints.
...
and Effects work together in OmniMux Clip or rendered Results: the graphics make the idea clear,
...
- **Project ownership.** Preserve unrelated Source, Recipe, Run, assets, and project-package work.
  Make production changes at their owning source; keep Result media intact.
- **Done means watched.** Watch the actual deliverable and judge it against the Brief, Treatment,
  ...
  and suitability for publishing. In OmniMux, open the editable multi-track project directly in
  OmniMux Clip so the user can inspect each track and make fine-tuned adjustments before final export.

## 66 篇制作手册与专项指南真源索引（Where the current question is answered）
...
| locating an existing execution installation, or installing or updating the executable after Skill setup | `references/environment/distribution.md` |
