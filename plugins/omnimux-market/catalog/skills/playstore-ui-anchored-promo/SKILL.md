...
4. **Use the typography-preserving image model.** In this environment that is `gpt-image-2`. Do **not** use nano-banana / nano-banana-2 for these composites — small UI typography inside the phone mockup garbles. If only nano-banana is available, surface this to the user before generating.
