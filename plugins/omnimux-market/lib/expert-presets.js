 ...
 */
export function healAgentPresetCordis(content) {
        ...
        if (/^-\s+id:\s+tool-subagent\b/.test(line)) {
            ...
            if (blockText.includes("'@deepseek-ai/dsh-tool-subagent'") || blockText.includes('"@deepseek-ai/dsh-tool-subagent"')) {
                if (!/^\s+provider:\s+/m.test(blockText)) {
                    const configIndex = block.findIndex((l) => /^\s+config:\s*$/.test(l));
                    if (configIndex >= 0) {
                        block.splice(configIndex + 1, 0, '    provider: spawn', '    toolName: subagent', '    modelSelectionSettings: true', '    backgroundMode: continuable');
                    }
                    else {
                        block.push('  config:', '    provider: spawn', '    toolName: subagent', '    modelSelectionSettings: true', '    backgroundMode: continuable');
                    }
                }
            }
            result.push(...block);
 ...
 */
export function healInstalledAgentPresets(home) {
