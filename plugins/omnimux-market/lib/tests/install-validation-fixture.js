...
export function zipFixture(entries) {
    ...
    for (const entry of entries) {
        ...
        if (entry.descriptor) {
            ...
            descriptor.writeUInt32LE(checksum, d);
            descriptor.writeUInt32LE(payload.length, d + 4);
            descriptor.writeUInt32LE(size, d + 8);
        }
...
export function extraField(id, data) {
    const header = Buffer.alloc(4);
    header.writeUInt16LE(id);
    header.writeUInt16LE(data.length, 2);
    return Buffer.concat([header, data]);
}
export function unicodeExtra(raw, name) {
    const header = Buffer.alloc(5);
    header[0] = 1;
    header.writeUInt32LE(crc32.unsigned(raw), 1);
