...
export function qaExtra(id, data) {
    const header = Buffer.alloc(4);
    header.writeUInt16LE(id);
    header.writeUInt16LE(data.length, 2);
    return Buffer.concat([header, data]);
}
export function qaUnicode(raw, name) {
    const prefix = Buffer.alloc(5);
    prefix[0] = 1;
    prefix.writeUInt32LE(qaCrc(raw), 1);
    return qaExtra(0x7075, Buffer.concat([prefix, Buffer.from(name)]));
}
export function qaZip(entries, comment = Buffer.alloc(0)) {
    const locals = [];
    const centrals = [];
    let offset = 0;
    for (const e of entries) {
        ...
        if (e.descriptor) {
            ...
            descriptor.writeUInt32LE(crc, p);
            descriptor.writeUInt32LE(payload.length, p + 4);
            descriptor.writeUInt32LE(data.length, p + 8);
        }
...
export function qaPackage(...entries) {
    return qaZip([{ name: 'SKILL.md', data: QA_SKILL }, ...entries]);
}
export function qaCentralOffsets(bytes) {
