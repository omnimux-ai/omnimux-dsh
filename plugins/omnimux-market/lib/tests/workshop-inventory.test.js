...
async function fixture(run) {
    ...
    try {
        await run(root, { scopeKey: 'scope', label: 'Fixture only', complete: true, reasons: [], roots: [{ id: 'fixture-root', path: root }] });
    }
    finally {
        await rm(root, { recursive: true, force: true });
    }
}
async function skill(root, token, text = md(token)) {
    await mkdir(join(root, token));
    await writeFile(join(root, token, 'SKILL.md'), text);
}
test('unknown scope is explicit error, not successful zero inventory', async () => {
    ...
    assert.deepEqual(result.reasons, ['SCOPE_UNVERIFIED']);
});
...
test('actual historical file is inventoried without guessing origin/version/time/enabled', async () => fixture(async (root, scope) => {
    ...
    assert.equal(row.skill.installed, true);
    assert.equal(row.skill.enabled, null);
    assert.equal(row.skill.version, null);
    assert.equal(row.skill.updatedAt, null);
    assert.equal(row.skill.sourceRef, null);
    assert.deepEqual(result.sourceOptions, ['all', 'unknown']);
    assert(!JSON.stringify(result).includes(root));
    assert(!JSON.stringify(result).includes('Synthetic body'));
...
test('proven installed origin persists independently from discover winner; policy is not enabled proof', async () => fixture(async (root, scope) => {
    ...
    assert.equal(result.records[0].origin, 'workbuddy');
    assert.equal(result.records[0].skill.enabled, null);
...
test('local files unchanged, no schema migration and revision invalidates after metadata edit', async () => fixture(async (root, scope) => {
    ...
    const b = await service.reconcile();
...
test('invalid packages retained as invalid and partial, not swallowed into empty', async () => fixture(async (root, scope) => {
    ...
    assert.equal(result.records[0].verification, 'invalid');
