...
import { OFFICIAL_SHELF_SCOPE, SKILL_BILINGUAL_FIELDS, SKILL_BILINGUAL_LIMITS, checkSkillBilingual, isOfficialShelfItem, normalizeBilingualField, } from '../skill-bilingual.js';
import { SKILL_BILINGUAL_LIMITS as CATALOG_BILINGUAL_LIMITS, catalogRoot, loadCatalog, parseCatalog, } from '../expert/catalog.js';
const complete = {
    titleZh: '三维动画短片生成',
...
test('T01-11：目录解析层不得静默截断标签（领域标签必须存活到工坊投影）', () => {
    const raw = JSON.parse(readFileSync(join(catalogRoot(), 'index.json'), 'utf-8'));
...
test('T01-10：真实目录的 69 项官方货架技能全部通过判据（T01 验收证据）', () => {
    ...
    const shelf = catalog.items.filter((item) => isOfficialShelfItem(item));
    ...
    const sample = catalog.items.find((item) => item.id === 'sk-omx-3d-animation-short-generator');
