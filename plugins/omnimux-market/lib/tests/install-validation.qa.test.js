...
import { VALIDATION_LIMITS as L } from '../install-validation-contract.js';
import { QA_SKILL, qaCrc, qaExtra, qaUnicode, qaZip, qaPackage, qaCentralOffsets } from './install-validation-qa-fixture.js';
const zip = (bytes, limits) => validateInstallPackage({ format: 'zip', fileName: 'qa.zip', bytes }, { limits });
const md = (bytes) => validateInstallPackage({ format: 'markdown', fileName: 'SKILL.md', bytes });
...
test('QA actual valid ZIP reaches exactly 20 MiB and +1 fails input admission', async () => {
    ...
    assert.equal(bytes.length, L.zipBytes);
    assert(second.length < L.resourceBytes);
    const result = await zip(bytes);
    assert(result.ok, JSON.stringify(result));
    assert.equal(result.inputBytes, L.zipBytes);
...
for (const [label, mutate] of [
    ...
    ['local header overlaps central', (b) => b.writeUInt16LE(65535, 28)],
    ['local method differs', (b) => b.writeUInt16LE(8, 8)],
    ['local flags differ', (b) => b.writeUInt16LE(0, 6)],
    ['local version differs', (b) => b.writeUInt16LE(10, 4)],
    ['central count lower', (b) => { b.writeUInt16LE(1, b.length - 14); b.writeUInt16LE(1, b.length - 12); }],
    ['central record points into payload', (b) => b.writeUInt32LE(40, qaCentralOffsets(b)[1] + 42)],
    ['ZIP64 local sentinel', (b) => b.writeUInt32LE(0xffffffff, 18)],
    ['central ZIP64 size', (b) => b.writeUInt32LE(0xffffffff, qaCentralOffsets(b)[0] + 24)],
    ['central disk mismatch', (b) => b.writeUInt16LE(1, b.length - 14)],
])
    test(`QA structure rejects ${label}`, async () => {
        const bytes = qaPackage({ name: 'r' });
        mutate(bytes);
        denied(await zip(bytes), 'PACKAGE_FORMAT');
    });
for (const descriptor of ['signed', 'unsigned'])
    test(`QA descriptor ${descriptor} local nonzero exact values are legal`, async () => {
        const bytes = qaZip([{ name: 'SKILL.md', data: QA_SKILL, descriptor }]);
        bytes.writeUInt32LE(qaCrc(QA_SKILL), 14);
        bytes.writeUInt32LE(QA_SKILL.length, 18);
        bytes.writeUInt32LE(QA_SKILL.length, 22);
...
test('QA ASI hardlink payload is rejected even when Unix mode says ordinary file', async () => {
    ...
    data.writeUInt16LE(0x81a4, 4);
    data[14] = 120;
    data.writeUInt32LE(qaCrc(data.subarray(4)), 0);
    denied(await zip(qaPackage({ name: 'r', extra: qaExtra(0x756e, data) })), 'UNSAFE_PATH');
});
for (const flag of [2, 4, 0x20, 0x100, 0x400, 0x1000, 0x8000])
    test(`QA STORE unsupported flag ${flag}`, async () => {
        denied(await zip(qaPackage({ name: 'r', flags: flag })), 'PACKAGE_FORMAT');
    });
for (const pair of [['ﬃ', 'ffi'], ['ſ', 'S'], ['Ꭰ', 'ꭰ'], ['𐐀', '𐐨'], ['A/x', 'a/y'], ['e\u0301/', 'é/'], ['a/b', 'a']]) {
...
test('QA directory denominator cannot rescue real deflate ratio above 100', async () => {
    const data = Buffer.alloc(1201, 97);
    assert.equal(deflateRawSync(data).length, 12);
    denied(await zip(qaPackage({ name: 'empty/', method: 8 }, { name: 'r', data, method: 8 })), 'PACKAGE_LIMIT');
});
for (const suffix of [Buffer.alloc(16384), deflateRawSync(Buffer.from('second stream'))])
    test(`QA real consumed bytes reject appended ${suffix.length} bytes`, async () => {
...
test('QA scripts hooks HTML and nested archive stay opaque bytes', async () => {
    ...
    assert.equal(result.manifest[3].bytes, nested.length);
});
for (const yaml of [
    ...
    test(`QA rejected YAML is redacted ${yaml.slice(0, 45)}`, async () => {
        const result = await md(Buffer.from(`---\n${yaml}\n---\nPRIVATE_BODY_QA /private/synthetic/qa`));
        denied(result, 'INVALID_SKILL');
        assert(!JSON.stringify(result).includes('PRIVATE_BODY_QA'));
        assert(!JSON.stringify(result).includes('/private'));
    });
test('QA legal folded description block and YAML end marker remain accepted', async () => {
    const result = await md(Buffer.from('---\nname: qa\ndescription: >-\n  first\n  second\n...\n---\nbody'));
    assert(result.ok, JSON.stringify(result));
    assert.equal(result.metadata.description, 'first second');
});
for (const bad of [Buffer.from([0xed, 0xa0, 0x80]), Buffer.from([0xf4, 0x90, 0x80, 0x80]), Buffer.from([0x80])])
    test(`QA UTF8 fatal ${bad.toString('hex')}`, async () => {
        denied(await md(Buffer.concat([QA_SKILL, bad])), 'INVALID_SKILL');
    });
for (const invalid of [NaN, Infinity, 0, -1, 1.5])
    test(`QA invalid reduced budget ${invalid}`, async () => {
...
test('QA inherited enlarged budget cannot override frozen defaults', async () => {
    const limits = Object.create({ zipBytes: L.zipBytes + 1 });
    denied(await zip(Buffer.alloc(L.zipBytes + 1), limits), 'PACKAGE_LIMIT');
    assert(Object.isFrozen(L));
});
test('QA SAB backing cannot be disguised by shadowing public buffer property', async () => {
    const bytes = new Uint8Array(new SharedArrayBuffer(QA_SKILL.length));
    bytes.set(QA_SKILL);
...
test('QA input accessor cannot substitute SAB after byte admission', async () => {
    const shared = new Uint8Array(new SharedArrayBuffer(QA_SKILL.length));
    shared.set(QA_SKILL);
    let reads = 0;
    const input = { format: 'markdown', fileName: 'SKILL.md', get bytes() { return ++reads <= 2 ? QA_SKILL : shared; } };
...
test('QA accessor cannot substitute a valid over-20MiB ZIP after admission', async () => {
    ...
    const small = qaPackage();
    let reads = 0;
    const input = { format: 'zip', fileName: 'qa.zip', get bytes() { return ++reads <= 3 ? small : oversized; } };
...
test('QA valid deep YAML within byte budget is bounded without invented depth policy', async () => {
    ...
    if (result.ok)
        assert.deepEqual(result.metadata, { name: 'qa', description: 'ok', version: null });
    else
        denied(result, 'INVALID_SKILL');
});
for (const kind of ['success', 'malformed', 'cancel'])
    test(`QA actual worker exits on ${kind} and abort listener is released`, async () => {
        const ids = new Set();
        const destroyed = new Set();
        const hook = createHook({ init(id, type) { if (type === 'WORKER')
                ids.add(id); }, destroy(id) { destroyed.add(id); } });
        hook.enable();
        try {
            ...
            if (kind === 'success')
                ...
                denied(result, kind === 'cancel' ? 'VALIDATION_ABORTED' : 'INVALID_SKILL');
            await immediate();
            assert.equal(ids.size, 1);
            for (const id of ids)
                assert(destroyed.has(id));
            controller.abort();
            await immediate();
        }
        finally {
