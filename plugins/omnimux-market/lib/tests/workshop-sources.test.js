...
const request = { view: 'discover', query: '', domain: 'all', source: 'all', queryRevision: 1, uninstalledOnly: false };
function sources(remotePage) { return { catalog: async () => catalog(), remotePage }; }
test('packaged real catalog is enumerated and fingerprinted without recommendation fabrication', async () => {
    ...
    assert.equal(result.catalog.items.filter((r) => r.kind === 'skill' && r.recommended).length, 70);
});
...
test('full paginated enumeration consumes all candidates before sorting and reports exact on supported stable provider', async () => {
    ...
    assert.equal(result.remote.length, 163);
...
test('20 pages and 1600 candidates stop real loader without requesting page 21', async () => {
    ...
    assert.equal(result.remote.length, 1600);
    assert.equal(result.status.code, 'SOURCE_LIMIT');
});
test('repeated identities/cursor and overlong page cannot inflate accepted results', async () => {
    ...
    assert.equal(repeated.remote.length, 1);
    assert.equal(repeated.status.code, 'SOURCE_NO_PROGRESS');
    ...
    assert.equal(long.remote.length, 80);
    assert.equal(long.status.code, 'SOURCE_LIMIT');
});
test('timeout aborts uncooperative provider and retains no false completeness', async () => {
    ...
    assert.equal(result.status.code, 'SOURCE_TIMEOUT');
});
...
test('actual SkillHub transport sends only query and fixed paging, keeps unknown evidence unknown', async () => {
    ...
    assert.equal(seen[0].pathname, '/api/skills');
    assert.equal(seen[0].searchParams.get('keyword'), 'fixture');
    assert.deepEqual([...seen[0].searchParams.keys()].sort(), ['keyword', 'order', 'page', 'pageSize', 'sortBy']);
    assert.equal(result.remote[0].downloads, 0);
    assert.equal(result.remote[0].sourceRef, null);
    assert.equal(result.remote[0].updatedAt, null);
...
test('empty upstream search never triggers popular fallback', async () => {
    let calls = 0;
    const api = createWorkshopSources(withDefaults({}), (async () => { calls++; return new Response(JSON.stringify({ code: 0, data: { skills: [], total: 0 } })); }));
    const result = await enumerateWorkshopRemote(api, 'unknown');
    assert.equal(calls, 1);
    assert.equal(result.remote.length, 0);
    assert.notEqual(result.status.status, 'complete');
});
test('full response byte limit and abort-aware body deadline are enforced', async () => {
    const huge = createWorkshopSources(withDefaults({}), (async () => new Response('x'.repeat(2 * 1024 * 1024 + 1))));
...
test('QueryService consumes snapshots: no remote browse, full exact count, revision and TTL rejection', async () => {
    ...
    const service = new QueryService(sources(async () => { calls++; throw new Error('unexpected'); }), { reconcile: async () => ({ ...inventory(), revision }) }, () => time);
    const first = await service.query(request);
    assert.equal(first.count.value, 100);
    assert.equal(first.items.length, 48);
    assert.equal(first.count.mode, 'exact');
...
test('snapshot eviction and catalog revision changes invalidate old pages', async () => {
    let rev = 'a';
    const src = sources(async () => { throw new Error('unused'); });
    src.catalog = async () => ({ ...catalog(), revision: rev });
    const service = new QueryService(src, { reconcile: async () => inventory() });
    const first = await service.query(request);
    for (let i = 0; i < 8; i++)
        await service.query({ ...request, queryRevision: i + 2 });
    await assert.rejects(service.query({ ...request, cursor: first.nextCursor }), /CURSOR_EXPIRED/);
    const fresh = await service.query(request);
    rev = 'b';
...
test('detail binds exact catalog source and returns honest metadata completeness', async () => {
    ...
    assert.equal(result.skill.title, 'Fixture 1');
    assert.equal(result.descriptionComplete, false);
