...
/** 官方系统预设的增强元数据字典 */
export const SYSTEM_PRESET_METADATA = {
    standard: {
        name: '代码开发',
 ...
 */
export function materializeEnabledMarketExperts(home) {
    healInstalledAgentPresets(home);
    const installed = [];
    for (const exp of DEFAULT_MARKET_EXPERTS) {
        if (exp.initialStatus !== 'enabled')
            continue;
        const presetDir = join(home, '.agent-presets', exp.id);
        if (existsSync(join(presetDir, 'preset.yml')))
            continue;
        if (getMarketExpertStatus(home, exp) !== 'enabled')
            continue;
        try {
...
function scanLocalPresets(home) {
    const presets = [];
 ...
 */
export function reconcileMarketExperts(home, officialPresets) {
    ...
    for (const p of effectivePresets) {
        if (!p.id || p.id === 'html-generator')
            continue; // 彻底过滤 html-generator
    ...
    const groupWeight = { ecommerce: 0, system: 1, custom: 2 };
