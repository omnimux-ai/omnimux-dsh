...
export const STATIC_MARKET_EXPERTS = [...BUILTIN_AGENT_PRESETS, ...DEFAULT_MARKET_EXPERTS];
// ─────────────────────────────────────────────────────────────
// 磁盘扫描
// ─────────────────────────────────────────────────────────────
/** 预设根目录（用户预设与 .retired 归档均在此）。 */
export function agentPresetRoot(home) {
 ...
 * @returns 是否内置
 */
export function isBuiltinAgentPreset(id) {
 ...
 * @returns 是否已安装
 */
export function isAgentPresetInstalled(home, id) {
 ...
 * @returns 原始预设 id
 */
export function retireEntryId(name) {
 ...
 * @returns 元信息或 null
 */
export function readAgentPresetMeta(presetFile) {
...
function parsePresetYaml(raw) {
    ...
    for (const line of raw.split(/\r?\n/)) {
        ...
        if (value)
            fallback[match[1]] = value;
    }
    return Object.keys(fallback).length > 0 ? fallback : null;
}
/** 由 id + preset.yml 元信息构造展示条目；缺名字时退回 id，保证卡片不空。 */
function buildScannedExpert(id, meta, initialStatus) {
...
function compareExpertOrder(a, b) {
    if (a.order !== b.order)
 ...
 */
export function scanInstalledAgentPresets(home) {
 ...
 */
export function scanRetiredAgentPresets(home) {
    const dir = join(agentPresetRoot(home), '.retired');
    if (!isDirectory(dir))
        return [];
    const items = [];
...
export function getMarketExpertStatus(home, exp) {
    ...
    if (isBuiltinAgentPreset(exp.id))
        return hasRetiredMarker(home, exp.id) ? 'disabled' : 'enabled';
    if (isAgentPresetInstalled(home, exp.id))
        return 'enabled';
    if (hasRetiredEntry(home, exp.id))
        return 'disabled';
 ...
 * @returns 合并后的条目
 */
export function mergeMarketExpertMeta(exp, meta) {
    if (!meta)
        return { ...exp };
    return {
        ...exp,
 ...
 * @returns 带状态与来源的条目列表
 */
export function listMarketExperts(home) {
    const entries = [];
    const seen = new Set();
 ...
 * @returns 专家定义或 null
 */
export function findMarketExpert(home, id) {
...
function buildPresetYml(exp) {
    return `name: ${exp.name}\ndescription: ${exp.description}\norder: ${exp.order}\n`;
}
function buildCordisYml(exp) {
 ...
 * @param id 预设 id
 */
function clearRetiredMarkers(home, id) {
    const dir = join(agentPresetRoot(home), '.retired');
    if (!isDirectory(dir))
        return;
    for (const name of retiredEntries(home)) {
 ...
 * @returns 是否发生了还原
 */
function restoreRetiredPreset(home, id) {
...
export function installMarketExpertPreset(home, exp) {
    ...
    if (!existsSync(join(dir, 'preset.yml')))
        writeFileSync(join(dir, 'preset.yml'), buildPresetYml(exp), 'utf8');
    if (!existsSync(join(dir, 'agent.cordis.yml')))
        writeFileSync(join(dir, 'agent.cordis.yml'), buildCordisYml(exp), 'utf8');
}
/**
 * 禁用（离职）一个专家预设。
 *
