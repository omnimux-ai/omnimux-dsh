...
export async function readWorkshopFile(root, name, limit) {
    if (!safeRelativePath(name))
        throw new WorkshopReadError('UNSAFE_PATH');
    ...
    try {
        ...
        if (st.ino !== before.ino || st.dev !== before.dev || !st.isFile() || st.size > limit)
            throw new WorkshopReadError('READ_LIMIT');
        ...
        if (currentRoot.ino !== rootIdentity.ino || currentRoot.dev !== rootIdentity.dev
            ...
            || currentFile.ino !== st.ino || currentFile.dev !== st.dev || currentFile.isSymbolicLink())
            throw new WorkshopReadError('READ_CHANGED');
        if (offset > limit)
            throw new WorkshopReadError('READ_LIMIT');
        if (after.size !== st.size || after.mtimeMs !== st.mtimeMs || after.ctimeMs !== st.ctimeMs || offset !== st.size
            || await checkedDirectory(root) !== canonical || await checkedDirectory(parent) !== parent)
            throw new WorkshopReadError('READ_CHANGED');
...
const revision = (value) => Number.isSafeInteger(value) && Number(value) >= 0;
const date = (value) => value === null || workshopDate(value) !== null;
const token = (value) => typeof value === 'string' && /^[a-z0-9][a-z0-9_-]{0,127}$/.test(value);
export function validSourceRef(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return false;
    const r = value;
    const fields = { catalog: ['kind', 'catalogId', 'revision'], skillhub: ['kind', 'identity', 'version'],
        local: ['kind', 'contentHash'], git: ['kind', 'sourceId', 'repo', 'path', 'ref', 'commit'] };
    if (typeof r.kind !== 'string' || !Object.hasOwn(fields, r.kind) || Object.keys(r).some((key) => !fields[r.kind].includes(key)))
        return false;
    switch (r.kind) {
...
export function parseWorkshopState(value, scopeKey) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        throw new WorkshopReadError('STATE_INVALID');
    const s = value;
    if (Number(s.schemaVersion) > 1)
        throw new WorkshopReadError('SCHEMA_NEWER');
    if (s.schemaVersion !== 1 || s.scopeKey !== scopeKey || !revision(s.revision)
        || !Array.isArray(s.records) || !Array.isArray(s.policyTombstones) || !s.preferences)
        throw new WorkshopReadError('STATE_INVALID');
    const p = s.preferences;
    if (typeof p.autoUpdate !== 'boolean' || !revision(p.revision) || p.revision > s.revision
        || !date(p.lastCheckAt) || !date(p.nextEligibleAt))
        throw new WorkshopReadError('STATE_INVALID');
    const ids = new Set(), paths = new Set(), keys = new Set();
    for (const r of s.records) {
        if (!r || !text(r.installId) || !token(r.token) || r.skillKey !== r.token || !WORKSHOP_ORIGINS.includes(r.origin)
            ...
            || ids.has(r.installId) || paths.has(r.relativePath) || keys.has(r.skillKey))
            throw new WorkshopReadError('STATE_INVALID');
        ids.add(r.installId);
        paths.add(r.relativePath);
        keys.add(r.skillKey);
    }
    const tombstones = new Set();
    for (const t of s.policyTombstones) {
        if (!t || t.scopeKey !== scopeKey || !token(t.token) || t.skillKey !== t.token || t.reason !== 'uninstalled'
            || !text(t.operationId) || !revision(t.revision) || t.revision > s.revision || tombstones.has(t.skillKey))
            throw new WorkshopReadError('STATE_INVALID');
...
export function emptyWorkshopState(scopeKey) {
...
export function compareAndSwapWorkshopState(current, expected, next) {
    const old = parseWorkshopState(current, current.scopeKey);
    const candidate = parseWorkshopState(next, old.scopeKey);
    if (!revision(expected) || old.revision !== expected || candidate.revision !== expected + 1)
        throw new WorkshopReadError('REVISION_CONFLICT');
    ...
    for (const t of old.policyTombstones) {
        ...
        if (!retained || JSON.stringify(retained) !== JSON.stringify(t))
            throw new WorkshopReadError('TOMBSTONE_PROTECTED');
    }
    return candidate;
}
...
export class WorkshopStore {
    scopeKey;
    directory;
    constructor(scopeKey, directory) {
    ...
    async read() {
        try {
            ...
            if (error.code === 'ENOENT')
                return emptyWorkshopState(this.scopeKey);
