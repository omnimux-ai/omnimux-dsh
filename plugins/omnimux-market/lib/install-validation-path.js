...
function foldingMap(value) {
    if (!(value instanceof Map))
        throw new Error('Invalid Unicode folding data');
    for (const [key, replacement] of value) {
        if (typeof key !== 'string' || typeof replacement !== 'string')
            throw new Error('Invalid Unicode folding data');
    }
    return value;
}
...
export function normalizePath(name, limits) {
    const directory = name.endsWith('/');
    const raw = directory ? name.slice(0, -1) : name;
    if (!raw || /[\\\x00-\x1f\x7f:<>"|?*]/u.test(raw) || raw.startsWith('/'))
        reject('UNSAFE_PATH', 'PATH_SYNTAX');
    const components = raw.split('/').map(part => part.normalize('NFC'));
    for (const part of components) {
        if (!part || part === '.' || part === '..' || /[. ]$/u.test(part)
            || /^(con|prn|aux|nul|com[1-9¹²³]|lpt[1-9¹²³])(?:\.|$)/iu.test(part))
            reject('UNSAFE_PATH', 'PATH_COMPONENT');
...
export function checkPathSet(entries) {
    ...
    for (const entry of entries) {
        ...
        for (let i = 1; i <= components.length; i++) {
            ...
            if (previous !== undefined && previous !== path)
                reject('UNSAFE_PATH', 'PATH_COLLISION');
            if (kinds.has(key) && kinds.get(key) !== directory)
                reject('UNSAFE_PATH', 'FILE_DIRECTORY_CONFLICT');
            if (i === components.length) {
                if (explicit.has(key))
                    reject('UNSAFE_PATH', 'DUPLICATE_PATH');
                explicit.add(key);
            }
