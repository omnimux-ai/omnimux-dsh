 ...
 */
import { PIXEL_AVATAR_ACCESSORIES, PIXEL_AVATAR_ACCESSORY_COLORS, PIXEL_AVATAR_BACKGROUNDS, PIXEL_AVATAR_CLOTHING, PIXEL_AVATAR_DEFAULT_RENDER_SIZE, PIXEL_AVATAR_EYE_COLORS, PIXEL_AVATAR_EYE_COVERING_ACCESSORIES, PIXEL_AVATAR_EYE_STYLES, PIXEL_AVATAR_FACIAL_HAIR, PIXEL_AVATAR_GRID, PIXEL_AVATAR_HAIR_COLORS, PIXEL_AVATAR_HAIR_STYLES, PIXEL_AVATAR_MOUTH_STYLES, PIXEL_AVATAR_SKIN_TONES, PIXEL_AVATAR_TOOTH_COLOR, } from './pixel-avatar-constants.js';
...
function createCells(fill) {
    ...
    for (let y = 0; y < PIXEL_AVATAR_GRID; y += 1)
        cells.push(new Array(PIXEL_AVATAR_GRID).fill(fill));
    return cells;
}
function paintRun(cells, x0, x1, y, color) {
    if (!color || y < 0 || y >= PIXEL_AVATAR_GRID)
        return;
...
function paintRuns(cells, runs, color) {
    ...
    for (const run of runs)
        paintRun(cells, run[0], run[1], run[2], color);
}
function paintPixels(cells, pixels, color) {
    if (!pixels || !color)
        return;
    for (const pixel of pixels)
        paintPixel(cells, pixel[0], pixel[1], color);
}
function paintBackdrop(cells, background, random) {
    for (let y = 0; y < PIXEL_AVATAR_GRID; y += 1) {
        for (let x = 0; x < PIXEL_AVATAR_GRID; x += 1)
            cells[y][x] = background.base;
    }
    ...
    if (random() < 0.5)
        paintPixel(cells, leftSide ? 2 : 13, sparkleY + 2, background.accent);
}
function paintHead(cells, skin) {
    paintRun(cells, 5, 10, 2, skin.base);
    for (let y = 3; y <= 10; y += 1)
        paintRun(cells, 4, 11, y, skin.base);
...
function paintBody(cells, skin, clothes) {
    ...
    for (let y = 13; y <= 15; y += 1)
        paintRun(cells, 3, 12, y, clothes.base);
    for (let y = 13; y <= 15; y += 1) {
...
function paintHair(cells, styleKey, hairColor, hatColor) {
    ...
    if (style.hat)
        paintRuns(cells, style.hair, hairColor.base);
}
...
function paintAccessory(cells, accessoryKey) {
    ...
    if (!accessory || accessoryKey === 'none')
        return;
...
function renderCellsToSvg(cells, size) {
    ...
    for (let y = 0; y < PIXEL_AVATAR_GRID; y += 1) {
        ...
        while (x < PIXEL_AVATAR_GRID) {
            const color = row[x];
            let runLength = 1;
            while (x + runLength < PIXEL_AVATAR_GRID && row[x + runLength] === color)
                runLength += 1;
