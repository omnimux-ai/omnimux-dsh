...
import { getFileNameLowLevel } from 'yauzl';
...
function requireBytes(bytes, start, length, end = bytes.length) {
    if (!Number.isSafeInteger(start) || start < 0 || start + length > end)
        reject('PACKAGE_FORMAT', 'STRUCTURE_BOUNDS');
}
/** Supplement only raw fields yauzl does not validate; yauzl remains the entry reader. */
export function inspectCentral(bytes, limits) {
    const candidates = [];
    ...
    if (candidates.length !== 1)
        reject('PACKAGE_FORMAT', 'EOCD');
    const end = candidates[0];
    if (end >= 20 && bytes.readUInt32LE(end - 20) === 0x07064b50)
        reject('PACKAGE_FORMAT', 'ZIP64');
    if (bytes.readUInt16LE(end + 4) || bytes.readUInt16LE(end + 6))
        reject('PACKAGE_FORMAT', 'MULTI_DISK');
    ...
    if (count === 65535 || size === 0xffffffff || offset === 0xffffffff)
        reject('PACKAGE_FORMAT', 'ZIP64');
    if (bytes.readUInt16LE(end + 8) !== count || offset + size !== end)
        reject('PACKAGE_FORMAT', 'CENTRAL_DIRECTORY');
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (bytes.readUInt32LE(cursor) !== 0x02014b50)
            reject('PACKAGE_FORMAT', 'CENTRAL_SIGNATURE');
        if (bytes.readUInt16LE(cursor + 34) !== 0)
            reject('PACKAGE_FORMAT', 'MULTI_DISK');
        for (const field of [20, 24, 42])
            if (bytes.readUInt32LE(cursor + field) === 0xffffffff)
                reject('PACKAGE_FORMAT', 'ZIP64');
...
function inspectExtras(raw) {
    ...
    while (cursor < raw.length) {
        ...
        if (fields.has(id))
            reject('PACKAGE_FORMAT', 'DUPLICATE_EXTRA');
        const data = raw.subarray(cursor + 4, cursor + 4 + length);
        if (id === 1)
            reject('PACKAGE_FORMAT', 'ZIP64');
        if (id === 0x0017 || id === 0x9901)
            reject('PACKAGE_FORMAT', 'ENCRYPTED');
        if (id === 0x000d) {
            if (length < 12)
                reject('PACKAGE_FORMAT', 'UNIX_EXTRA');
            if (length > 12)
                reject('UNSAFE_PATH', 'UNIX_LINK_OR_DEVICE');
        }
        if (id === 0x756e) {
            if (length < 14 || crc32.unsigned(data.subarray(4)) !== data.readUInt32LE(0))
                reject('PACKAGE_FORMAT', 'ASI_EXTRA');
            const kind = data.readUInt16LE(4) & 0xf000;
            if ((kind !== 0x8000 && kind !== 0x4000) || data.readUInt32LE(6) !== 0 || length !== 14)
                reject('UNSAFE_PATH', 'ASI_LINK_OR_DEVICE');
        }
        if (id === 0x000a) {
            if (length < 4)
                reject('PACKAGE_FORMAT', 'NTFS_EXTRA');
            let pos = 4;
            while (pos < length) {
...
export function decodeName(raw, flags, extra) {
    if (!Buffer.isBuffer(raw) || !Buffer.isBuffer(extra))
        reject('PACKAGE_FORMAT', 'RAW_NAME_REQUIRED');
    ...
    if (unicode.length < 5 || unicode[0] !== 1 || unicode.readUInt32LE(1) !== crc32.unsigned(raw))
        reject('UNSAFE_PATH', 'UNICODE_EXTRA');
    const name = strictUtf8(unicode.subarray(5), 'UNSAFE_PATH');
    if (utf8 && name !== base)
        reject('UNSAFE_PATH', 'ENCODING_CONFLICT');
    // ASCII raw bytes have no legacy encoding ambiguity to justify a different spelling.
    if (!utf8 && raw.every(byte => byte < 128) && name !== base)
        reject('UNSAFE_PATH', 'ENCODING_CONFLICT');
    return name;
}
export function inspectEntry(entry) {
    ...
    if (flags & (1 | 0x40 | 0x2000))
        reject('PACKAGE_FORMAT', 'ENCRYPTED');
    if (flags & ~(0x800 | 8 | 6))
        reject('PACKAGE_FORMAT', 'UNSUPPORTED_FLAGS');
    if (entry.compressionMethod !== 0 && entry.compressionMethod !== 8)
        reject('PACKAGE_FORMAT', 'UNSUPPORTED_METHOD');
    if (entry.compressionMethod === 0 && (flags & 6))
        reject('PACKAGE_FORMAT', 'UNSUPPORTED_FLAGS');
    if (entry.versionNeededToExtract > 20)
        reject('PACKAGE_FORMAT', 'UNSUPPORTED_VERSION');
    ...
    if (kind !== 0 && kind !== 0x8000 && kind !== 0x4000)
        reject('UNSAFE_PATH', 'SPECIAL_FILE');
    if (entry.externalFileAttributes & 8)
        reject('UNSAFE_PATH', 'VOLUME_LABEL');
    if ((kind === 0x4000 || (entry.externalFileAttributes & 0x10)) && !name.endsWith('/'))
        reject('PACKAGE_FORMAT', 'DIRECTORY_ATTRIBUTES');
    if (kind === 0x8000 && name.endsWith('/'))
        reject('PACKAGE_FORMAT', 'DIRECTORY_ATTRIBUTES');
    return name;
}
/** Validate local metadata and a descriptor at the verified payload endpoint, never by scanning. */
export function inspectLocal(bytes, entry, local, centralStart) {
    const start = entry.relativeOffsetOfLocalHeader;
    requireBytes(bytes, start, 30, centralStart);
    const fields = ['versionNeededToExtract', 'generalPurposeBitFlag', 'compressionMethod', 'lastModFileTime', 'lastModFileDate'];
    if (fields.some(key => local[key] !== entry[key]) || !local.fileName.equals(entry.fileNameRaw))
        reject('PACKAGE_FORMAT', 'LOCAL_MISMATCH');
    ...
    if (localName !== centralName)
        reject('UNSAFE_PATH', 'LOCAL_ENCODING_MISMATCH');
    ...
    const values = ['crc32', 'compressedSize', 'uncompressedSize'];
    if (!(entry.generalPurposeBitFlag & 8)) {
        if (values.some(key => local[key] !== entry[key]))
            reject('PACKAGE_FORMAT', 'LOCAL_SIZE_CRC');
    }
    else {
        if (values.some(key => local[key] !== 0 && local[key] !== entry[key]))
            reject('PACKAGE_FORMAT', 'LOCAL_PLACEHOLDER');
        const matches = [];
        for (const signed of [false, true]) {
            const offset = end + (signed ? 4 : 0);
            if (offset + 12 > centralStart || (signed && bytes.readUInt32LE(end) !== 0x08074b50))
                continue;
            if (values.every((key, i) => bytes.readUInt32LE(offset + i * 4) === entry[key]))
                matches.push(offset + 12);
        }
        if (matches.length !== 1)
            reject('PACKAGE_FORMAT', 'DATA_DESCRIPTOR');
        end = matches[0];
    }
    return [start, end];
}
