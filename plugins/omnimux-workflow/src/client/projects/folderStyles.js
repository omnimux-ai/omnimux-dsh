...
.omnimux-page-card .omnimux-page-card-menu { bottom: 44px; right: 8px; }
.omnimux-folder-menu { --dsw-alias-bg-elevated: var(--dsw-specific-menu, var(--dsw-alias-bg-base)); position: absolute; z-index: 4; bottom: 20%; right: 5%; display: flex; flex-direction: column; gap: 2px; min-width: 148px; padding: 4px; border: 1px solid var(--dsw-alias-border-l2); border-radius: 10px; background: var(--dsw-alias-bg-elevated, var(--dsw-alias-bg-overlay)); -webkit-backdrop-filter: blur(16px); backdrop-filter: blur(16px); box-shadow: 0 10px 28px var(--dsw-alias-bg-mask-1), 0 2px 8px var(--dsw-alias-bg-mask-1); animation: omnimux-menu-pop 120ms cubic-bezier(0.16, 1, 0.3, 1); transform-origin: bottom right; }
