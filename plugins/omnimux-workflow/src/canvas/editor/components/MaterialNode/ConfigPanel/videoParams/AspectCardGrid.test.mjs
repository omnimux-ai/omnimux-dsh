 ...
 * （2026-09-07 全模态收敛 / T03：AspectCardGrid / DurationGrid / ChoiceTile /
 *   CompactToggle 已升格为 cfg/ 通用实现，videoParams 仅 re-export；
 *   控件本体断言指向 ../cfg 源码，re-export 路径另作断言。）
 *
 ...
 * 2. CfgAspectGrid 含 4 列网格类名与 wf-cfg-aspect-card 卡片结构；
 * 3. SegmentControls 生成方式分段消费 effective operations，≤1 隐藏；
 * 4. CfgDurationGrid 含 wf-cfg-duration-pill 与 onChange 透传；
...
const aspectSrc = readFileSync(join(here, '../cfg/CfgAspectGrid.tsx'), 'utf8');
const durationSrc = readFileSync(join(here, '../cfg/CfgDurationGrid.tsx'), 'utf8');
const aspectReExportSrc = readFileSync(join(here, 'AspectCardGrid.tsx'), 'utf8');
const durationReExportSrc = readFileSync(join(here, 'DurationGrid.tsx'), 'utf8');
...
test('CfgAspectGrid 采用 4 列网格类名与 wf-cfg-aspect-card 卡片结构', () => {
  assert.match(aspectSrc, /wf-cfg-aspect-grid/);
  assert.match(aspectSrc, /grid-template-columns:\s*repeat\(4,\s*1fr\)/);
  assert.match(aspectSrc, /wf-cfg-aspect-card/);
  assert.match(aspectSrc, /wf-cfg-aspect-card--active/);
  ...
  // videoParams 门面 re-export 保持既有引用路径
  assert.match(aspectReExportSrc, /CfgAspectGrid as AspectCardGrid/);
});
...
test('CfgDurationGrid 含时长胶囊网格类与 onChange 透传', () => {
  assert.match(durationSrc, /wf-cfg-duration-grid/);
  assert.match(durationSrc, /wf-cfg-duration-pill/);
  assert.match(durationSrc, /wf-cfg-duration-pill--active/);
  assert.match(durationSrc, /repeat\(auto-fill,\s*minmax\(56px,\s*1fr\)\)/);
  assert.match(durationSrc, /onChange\(opt\.value\)/);
  // videoParams 门面 re-export 保持既有引用路径
  assert.match(durationReExportSrc, /CfgDurationGrid as DurationGrid/);
});
