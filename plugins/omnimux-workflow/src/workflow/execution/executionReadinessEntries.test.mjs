...
test('HTTP and Agent reject a missing required prompt without reviving historical source text', async () => {
  let creates = 0;
  ...
  const deps = { store: { get: () => snapshot }, mediaDir: '/tmp', getCatalog: () => catalog, executionManager: { createExecution: () => { creates++; } } };
  ...
  assert.equal(agent.reasonCode, 'prompt_required');
