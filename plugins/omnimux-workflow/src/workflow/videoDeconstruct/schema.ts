...
function invalid(message: string): never {
  throw new VideoDeconstructError('invalid-request', message, 400);
}

export interface DeconstructVideoRequest {
  nodeId: string;
  videoPath: string;
  title?: string;
}

export function parseDeconstructVideoRequest(raw: unknown): Required<DeconstructVideoRequest> {
...
export interface DeconstructVideoResult {
  tableId: string;
  tablePath: string;
  title: string;
  columnCount: number;
...
export interface DeconstructVideoResponse {
  ok: boolean;
  code: number;
  data: DeconstructVideoResult | null;
  message: string;
  error?: string;
  tableId?: string;
  tablePath?: string;
