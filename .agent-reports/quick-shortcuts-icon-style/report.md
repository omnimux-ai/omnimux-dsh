...
**一处与任务前置说明不一致、已按用户原话修正的事实**：这一排此前**并不在输入框下方，而是在输入框上方**（官方 hero 栈把 `conversation.input.dock` 排在输入框之前）。用户原话第一条是「这一排按钮**应该放在输入框下方**」，因此本次显式把它排到输入框之后（`order: 3`），实测输入框 `202→316`、这一排 `334→364`，中间 18px（与 hero 栈既有间距同值）。第 6 节给出证据与回退方式。

独立评审（`review.md`）结论为「有建议、无阻塞」（4 中 5 低），其中 4 条中等问题与 3 条低优先项已在评审后修复（含评审当场抓出的一处幽灵 token 回退值），处置逐条见第 8 节；修复后已重建、重跑全部检查并在真实浏览器**重新取了一整套证据**（本节所有数字均为修复后最终构建的实测）。
...
| 命令 | 结果 |
| --- | --- |
| `node --test plugins/omnimux/src/client/composer-quick-shortcuts/*.test.js` | **60 通过 / 0 失败**（改动前 47；新增 `styles.test.js` 12 条 + `catalog.test.js` 加 2 条） |
| `node --test plugins/omnimux/tests/e2e/composer-quick-shortcuts-icon-style.e2e.test.mjs` | **4 通过 / 0 失败**（新增） |
| 变异验证（证明断言不是「换个写法就变绿」） | 把 `icons.jsx` 里 `film` 的一条路径 `M7 3v18` 改成 `M7 3v17` 后：单测 **11/12（1 红）**、E2E **3/4（1 红）**；改回后恢复 60/60 与 4/4 |
...
| `node scripts/verify-stage-contracts.mjs` | 13 个 Stage 组件审计后有 **2 条既有红灯**：`omnimux-social-harvest/src/client/HarvestStage.jsx` 缺 `injectStyles()`、`omnimux-accounts` 侧栏模式 `gui !== split`。把本轮改动 `git stash` 后在**同一基线上重跑，同名同结果**，故与本轮改动无关；本轮只改 `composer-quick-shortcuts/` 下文件，不在这两个插件内 |
...
| `06-窄列-与输入框等宽且不压字.png` | 640px 视口：这一排与输入框卡**等宽**（各 318px）、按钮折行、控件不溢出 |
| `特写A/B/C/D-*.png` | 上述关键状态的放大特写 |
| `browser-geometry.json` | 全部实测几何与计算样式（唯一证据真源） |

目视结论（均由实测几何支撑，非目测印象；数字为评审意见处置后**最终构建** 3188392 字节、中文界面 1920×929）：

- **在输入框下方**：基础态 输入框 `y 202→316`、这一排 `334→364`；选中态 输入框 `202→390`、这一排 `408→476`；间距恒为 18px（`belowInputBox: true`）。
- **整行居中**：行 `x 623 w 952`（中心 1099），四条组 `x 811.5→1386.5`（中心 1099）→ 偏差 **0px**；输入框卡中心同为 1099。
- **与输入框等宽**：宽列两处都是 952px；窄列 640px 下这一排 `x 20 w 318` 与输入框卡 `x 20 w 318` **完全对齐**（`rowMatchesCardWidth: true`）——评审 L2 的 12px 外溢已消除。
...
5. **位置规则的残留脆弱点**（评审 L1）：这一排落在输入框下方完全依赖 `order: 3` 这条规则；它假定「本行与输入框块是 hero 栈里的同级 flex 项」且「输入框那块 order 为 2」。任一宿主结构变化都会让它**静默**退回输入框上方。本轮已把这两个前提与失效方式写进源码注释、并按实测留下几何证据，但没有引入自动化几何断言——那需要完整应用环境（本任务未把它做进 CI）。若产品认为这条位置是硬要求，建议后续开任务补一条真实浏览器几何用例。
6. **`tests/e2e/` 的收集口径**（评审 M1）：包内 `run-tests.mjs` 只收 `src/**/*.test.js|ts`，CI 也不跑 `tests/e2e/`，同目录已有 54 个同类文件同样不被收集。本任务按最小口径处置（把显式命令写进规格），收集器口径建议另开任务统一裁决，否则「有 E2E 文件」会被误当作「CI 真的跑了」。

## 8. 独立评审意见的处置

独立评审（`review.md`，结论「有建议、无阻塞」，4 中 5 低）已逐条处置：

| 意见 | 处置 |
| --- | --- |
| M1 新增 E2E 无自动化执行入口 | **接受事实、按最小口径处置**：包内测试发现器只收 `src/**/*.test.js|ts`，CI 也无 `tests/e2e` 收集——同目录已有 54 个同类文件同样不被收集，属既有口径问题。本任务不擅自改收集器（会牵动 54 个文件的红灯风险），改为把该文件的显式命令写进规格「命令」一节并在本报告登记；收集器口径建议另开任务统一裁决 |
| M2 行尾箭头图标名与真源脱钩 | **已修**：`icons.jsx` 改为 `import { QUICK_SHORTCUT_ARROW_ICON } from './catalog.js'` 并按该常量取图；`styles.test.js` 改为断言「引用了真源常量、不得再出现字面量」 |
| M3 「逐字落地」断言只做抽样 | **已修**：`styles.test.js` 与 E2E 双双升级为**逐元素逐属性**对拍（五枚图标的每一个 `d`/`cx`/`cy`/`r`/`width`/`height`/`x`/`y`/`rx` 全部列入期望值，任一数值被改都会红） |
| M4 规格写的方案与实现漂移 | **已修**：规格的「居中实现方式」改写为实际方案（控件 `flex-basis: 100%` 自成一行），并补「位置」一节；S9 判据同步改写 |
| L1 位置依赖宿主 `order`、失效时静默 | **接受为残留风险**：`styles.js` 注释写明它依赖的两个宿主事实（同级 flex 项、输入框块 order 为 2）与失效方式，位置另有浏览器几何证据；真正的几何回归断言需要完整应用环境，本任务不引入（登记在第 7 节） |
| L2 窄列下这一排比输入框左右各宽 12px | **已修**：行宽改为 `min(卡片最大宽, 100% - 2×宿主侧边清除量)`，窄列下与输入框卡等宽 |
| L3 颜色 token 无回退值 | **改为硬门禁并抓到一处真问题**：新增断言「样式表引用的每个宿主 `--dsw-alias-*` 必须存在于宿主 Token 快照」，当场发现 `:focus-visible` 的回退值 `--dsw-alias-border-hover` **宿主并不存在**（浏览器实测该变量为空值），已删掉这个幽灵回退 |
| L4 注释里的实测数字未标状态 | **已修**：注释补基础态与选中态两组数字，并写明状态与环境 |
| L5a 选中态与悬停只差字重 | **保留**（规格 S8 允许，观感交人工真机复核）；L5b 三个常量导出收窄为模块内常量，对外只留两个组件 |
