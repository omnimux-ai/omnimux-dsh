...
export async function startFixture() {
  ...
  // Optional read-only override for worktrees whose file dependency is not linked.
  ...
  const output = await build({
    absWorkingDir: root,
    entryPoints: [resolve(import.meta.dirname, 'inspiration-text-layout.fixture.jsx')],
    bundle: true,
    write: false,
    outdir: resolve(root, 'tmp/layout-e2e-build'),
    format: 'iife',
    jsx: 'automatic',
  ...
  const identity = {
    ...
    pid: process.pid,
    bundle: hash(js),
    files: Object.fromEntries(['InspirationPreviewModal.jsx', 'styles.js', 'inspiration-preview-data.js'].map(path => [
      path, hash(readFileSync(resolve(root, 'plugins/omnimux-inspiration/src/client', path))),
    ])),
  }
