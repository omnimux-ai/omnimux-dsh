...
for(const width of [1440,390]) {
 ...
 const mobileTabsDisplay=await p.evaluate(()=>{const el=document.querySelector('.omnimux-inspiration-modal-mobile-tabs');return el?window.getComputedStyle(el).display:'none';});
 if(width===1440){assert.equal(mobileTabsDisplay,'none','桌面端大屏必须彻底隐藏移动端切换标签栏');}
 if(width===390){assert.equal(mobileTabsDisplay,'flex','窄屏移动端下必须正常展示切换标签栏');const match=snapshot.match(/tab \\[ref=(\\d+)\\]\\n\\s+text "内容解构"/);assert.ok(match,'真实快照需包含内容解构tab');await p.click('@'+match[1]);}
