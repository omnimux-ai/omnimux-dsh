...
test('E2E: free-ai-avatars 云端入库与「类型」维度落架闭环验证', async () => {
  ...
  assert.equal(charCat.total, 429, '角色总数必须为 429 套 (329 原有数字人 + 100 款新增人设)');
  ...
  assert.ok(optionValues.has('digital-human'), '必须包含 digital-human (老数据大一统统称)');
  assert.ok(optionValues.has('Human Archetypes'), '必须包含原生分类 Human Archetypes');
  ...
  assert.ok(sampleNew.meta.source_cover_url.startsWith('https://media.aftermark.ai/'), '必须绑定 R2 CDN 封面地址');
  assert.ok(sampleNew.meta.source_media_url.startsWith('https://media.aftermark.ai/'), '必须绑定 R2 CDN 音频地址');
  assert.equal(sampleNew.meta.dims.type, 'Animal Character', '角色类型维度必须正确写入原生分类');
});
