...
try{
 ...
 await p.evaluate(()=>window.__panel.patch('正文甲',{generatedContent:'',content:'OLD'}));await blocked('waiting');
 ...
 await p.evaluate(()=>window.__panel.patch('target',{slotBindings:{removed:[{sourceNodeId:'图片甲',edgeId:'edge-1',outputId:location.origin+'/media/a.svg',pinned:true}]}}));await blocked('conflict');
